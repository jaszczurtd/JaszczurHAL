...
static void printStats(void) {
  ...
  if (!hal_kv_get_stats(&stats)) {
    derr("KV: stats unavailable");
    return;
  }

  deb("KV: gen=%lu used=%u/%u keys=%u next=%lu",
      (unsigned long)stats.generation, stats.used_bytes, stats.capacity_bytes,
      stats.key_count, (unsigned long)stats.next_sequence);
}
...
static void initKvStore(void) {
  ...
  hal_kv_set_blob(KEY_DEVICE_NAME, (const uint8_t *)device_name,
