...
static void test_epoch_conversion(void) {
    ...
    hal_deb("After +3600s: %llu (expected ~%llu, diff=%lld)\r\n",
                (unsigned long long)epoch_verify,
                (unsigned long long)epoch_out,
...
void app_start(void) {
    ...
    const hal_rtc_config_t cfg = {
        ...
        .bus.i2c = {
            .i2c_bus = 0,
            .sda_pin = 4,           /* RP2040: GPIO4, STM32G474: PB9 (ignored in STM32 HAL) */
            .scl_pin = 5,           /* RP2040: GPIO5, STM32G474: PB8 (ignored in STM32 HAL) */
    ...
    if (hal_rtc_get_datetime(g_rtc, &now)) {
        ...
        if (!ok) {
            hal_rtc_datetime_t default_time = {
                .year = 2025,
                .month = 1,
                .day = 1,
                .hour = 12,
