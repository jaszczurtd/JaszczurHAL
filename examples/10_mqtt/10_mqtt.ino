...
static void onMqttMessage(const char *topic,
  ...
  char text[96] = {0};
  const uint16_t copy_len = length < (sizeof(text) - 1u)
                          ? length
                          : (uint16_t)(sizeof(text) - 1u);
  for (uint16_t i = 0; i < copy_len; ++i) {
    text[i] = (char)payload[i];
  }

  hal_deb("MQTT RX %s: %s", topic ? topic : "", text);
}
...
static void connectMqtt(void) {
  ...
  hal_deb("MQTT: connecting to %s:%u", MQTT_HOST, MQTT_PORT);
  if (hal_mqtt_connect(MQTT_CLIENT_ID)) {
    hal_deb("MQTT: connected");
    hal_mqtt_subscribe(MQTT_TOPIC_SUB, 0);
  } else {
    hal_derr("MQTT: connect failed, state=%d", hal_mqtt_state());
  }
}
...
static void publishTelemetry(void) {
  ...
  hal_deb("MQTT TX %s: %s", MQTT_TOPIC_PUB, payload);
}
