...
#if defined(HAL_ENABLE_MCP9600)
    if (mcp9600) {
        hal_deb("MCP9600: hot=%.2f C ambient=%.2f C",
                hal_thermocouple_read(mcp9600),
...
#if defined(HAL_ENABLE_MAX6675)
    if (max6675) {
        hal_deb("MAX6675: hot=%.2f C", hal_thermocouple_read(max6675));
    }
#endif
}
