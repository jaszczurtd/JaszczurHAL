...
#error "18_freertos_suite requires HAL_ENABLE_APP_TASK1"
#endif

#if HAL_TARGET_IS_RP && !defined(__FREERTOS)
#error "18_freertos_suite on RP targets requires FreeRTOS mode (__FREERTOS)"
#endif
...
static void smoke_table_worker(void *arg) {
  ...
  for (;;) {
    ...
    if (xSemaphoreTake(s_table_mutex, portMAX_DELAY) == pdTRUE) {
      ...
      deb("18_freertos_suite: %s slot=%lu read=%lu neighbor=%lu write=%lu "
...
void app_start(void) {
  ...
  if (s_smoke_mutex == NULL) {
    derr("18_freertos_suite: mutex allocation failed");
    return;
  }

  s_table_mutex = xSemaphoreCreateMutex();
  if (s_table_mutex == NULL) {
    derr("18_freertos_suite: table mutex allocation failed");
    return;
  }
  ...
  if (created != pdPASS) {
    derr("18_freertos_suite: xTaskCreate worker_a failed");
    return;
  }
  ...
  if (created != pdPASS) {
    derr("18_freertos_suite: xTaskCreate worker_b failed");
    return;
  }

  deb("18_freertos_suite: started two FreeRTOS table workers");
}
