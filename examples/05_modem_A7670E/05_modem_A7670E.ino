...
static bool modemInit(void) {
  ...
  if (modem == nullptr) {
    ...
    hal_modem_at_set_tick_callback(hal_simcom_a76xx_get_at(modem), modemTick, nullptr);
  ...
  if (hal_simcom_a76xx_wait_network_registered(modem, 60000) != HAL_SIMCOM_A76XX_OK) {
    return false;
  }
...
static bool mqttPublish(const char *topic, const char *payload) {
  return (hal_simcom_a76xx_mqtt_publish(modem, 0, topic,
...
void setup() {
  ...
  if (!modemInit()) {
    hal_derr("modem init failed");
    return;
  }
  if (!mqttConnect()) {
    hal_derr("mqtt connect failed");
    return;
  }
}
...
void loop() {
  ...
  if ((modem != nullptr) && (now - last_publish_ms >= 10000u)) {
    last_publish_ms = now;
...
void setup() {
  hal_derr("Enable HAL_ENABLE_A7670 to build this example");
}
