...
void app_start(void) {
  ...
  const hal_timer_result_t create_result =
      hal_timer_create(HAL_TIMER_POOL_DEFAULT, 250000u, true, onPeriodicTimer,
                       NULL, &periodic_timer);
...
void app_task0(void) {
  ...
  deb("timer ticks=%lu state=%d remaining=%ld result=%d",
      (unsigned long)timer_ticks, (int)hal_timer_get_state(periodic_timer),
      (long)remaining_us, (int)remaining_result);
}
