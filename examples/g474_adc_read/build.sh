#!/usr/bin/env bash
# Build the STM32G474 ADC reader (Nucleo-G474RE) into a flashable ELF/BIN/HEX.
...
echo "  LD  g474_adc_read.elf"
arm-none-eabi-g++ ${ARCH} -T"${LD}" -nostartfiles -Wl,--gc-sections \
    --specs=nano.specs --specs=nosys.specs "${OBJS[@]}" -o "${OUT}/g474_adc_read.elf"
arm-none-eabi-objcopy -O binary "${OUT}/g474_adc_read.elf" "${OUT}/g474_adc_read.bin"
arm-none-eabi-objcopy -O ihex   "${OUT}/g474_adc_read.elf" "${OUT}/g474_adc_read.hex"
arm-none-eabi-size "${OUT}/g474_adc_read.elf"
