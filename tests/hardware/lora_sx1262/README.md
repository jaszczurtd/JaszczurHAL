...
over-the-air packets, sequence continuity, RSSI/SNR metadata, sleep/wake and
radio destroy/create reinitialization.
...
`HAL_OK` sleep/wake and `HAL_OK` reinitialization. Then swap which physical
device receives the `responder` build and repeat.

Repeat the gate with `HAL_LORA_EXAMPLE_SF` and
`HAL_LORA_EXAMPLE_TX_POWER_DBM` supplied as project extra definitions for at
least SF7/6 dBm, SF9/10 dBm and SF12/14 dBm. Both ends of a run must use the
same values. Record module/antenna labels, exact wiring, firmware revision,
distance, packet counts, loss, RSSI/SNR range and the verifier JSON in the
private hardware report.
