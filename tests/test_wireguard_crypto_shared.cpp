...
void test_blake2s_matches_known_vector_for_abc(void) {
    ...
    TEST_ASSERT_EQUAL_INT(0, blake2s(digest,
                                     sizeof(digest),
                                     NULL,
                                     0u,
                                     msg,
...
void test_x25519_matches_rfc7748_vectors(void) {
    ...
    TEST_ASSERT_TRUE(hex_to_bytes(
        ...
        alice_public_expected, sizeof(alice_public_expected)));
    TEST_ASSERT_TRUE(hex_to_bytes(
    ...
    TEST_ASSERT_TRUE(hex_to_bytes(
        ...
        bob_public_expected, sizeof(bob_public_expected)));
    TEST_ASSERT_TRUE(hex_to_bytes(
        "4a5d9d5ba4ce2de1728e3bf480350f25"
        "e07e21c947d19e3376f09b3c1e161742",
        shared_expected, sizeof(shared_expected)));

    TEST_ASSERT_EQUAL_INT(0, x25519(alice_public, alice_private, X25519_BASE_POINT, 1));
    TEST_ASSERT_EQUAL_INT(0, x25519(bob_public, bob_private, X25519_BASE_POINT, 1));
    TEST_ASSERT_EQUAL_INT(0, memcmp(alice_public_expected, alice_public, sizeof(alice_public)));
    TEST_ASSERT_EQUAL_INT(0, memcmp(bob_public_expected, bob_public, sizeof(bob_public)));
    ...
    TEST_ASSERT_EQUAL_INT(0, memcmp(shared_expected, shared_ab, sizeof(shared_ab)));
    TEST_ASSERT_EQUAL_INT(0, memcmp(shared_expected, shared_ba, sizeof(shared_ba)));
}
...
void test_chacha20poly1305_roundtrip_and_tag_validation(void) {
    ...
    chacha20poly1305_encrypt(cipher,
                             plain,
                             sizeof(plain),
                             aad,
                             sizeof(aad),
    ...
    TEST_ASSERT_TRUE(chacha20poly1305_decrypt(decrypted,
                                              cipher,
                                              sizeof(cipher),
                                              aad,
                                              sizeof(aad),
    ...
    TEST_ASSERT_FALSE(chacha20poly1305_decrypt(decrypted,
                                               tampered,
                                               sizeof(tampered),
                                               aad,
                                               sizeof(aad),
...
void test_chacha20poly1305_ietf_detached_rejects_invalid_arguments(void) {
    ...
    TEST_ASSERT_FALSE(chacha20poly1305_encrypt_ietf_detached(NULL, tag, plain, 1u, aad, 1u, nonce, key));
    TEST_ASSERT_FALSE(chacha20poly1305_encrypt_ietf_detached(cipher, tag, NULL, 1u, aad, 1u, nonce, key));
    TEST_ASSERT_FALSE(chacha20poly1305_encrypt_ietf_detached(cipher, tag, plain, 1u, NULL, 1u, nonce, key));
    TEST_ASSERT_FALSE(chacha20poly1305_encrypt_ietf_detached(cipher, NULL, plain, 1u, aad, 1u, nonce, key));
    TEST_ASSERT_FALSE(chacha20poly1305_encrypt_ietf_detached(cipher, tag, plain, 1u, aad, 1u, NULL, key));
    TEST_ASSERT_FALSE(chacha20poly1305_encrypt_ietf_detached(cipher, tag, plain, 1u, aad, 1u, nonce, NULL));

    TEST_ASSERT_FALSE(chacha20poly1305_decrypt_ietf_detached(NULL, cipher, 1u, tag, aad, 1u, nonce, key));
    TEST_ASSERT_FALSE(chacha20poly1305_decrypt_ietf_detached(out, NULL, 1u, tag, aad, 1u, nonce, key));
    TEST_ASSERT_FALSE(chacha20poly1305_decrypt_ietf_detached(out, cipher, 1u, NULL, aad, 1u, nonce, key));
    TEST_ASSERT_FALSE(chacha20poly1305_decrypt_ietf_detached(out, cipher, 1u, tag, NULL, 1u, nonce, key));
    TEST_ASSERT_FALSE(chacha20poly1305_decrypt_ietf_detached(out, cipher, 1u, tag, aad, 1u, NULL, key));
    TEST_ASSERT_FALSE(chacha20poly1305_decrypt_ietf_detached(out, cipher, 1u, tag, aad, 1u, nonce, NULL));
