...
void test_adcCompe_zero(void) { TEST_ASSERT_EQUAL_INT(0, adcCompe(0)); }
...
void test_rgb888ToRgb565_buffer(void) {
  const unsigned char rgb[] = {
      255, 0, 0, 0, 255, 0, 0, 0, 255, 255, 255, 255,
  };
  unsigned short out[4] = {0};

  TEST_ASSERT_TRUE(rgb888ToRgb565(rgb, out, 4));
...
void test_rgba8888ToRgb565_buffer_ignores_alpha(void) {
  const unsigned char rgba[] = {
      255, 0, 0, 0, 0, 255, 0, 64, 0, 0, 255, 128, 255, 255, 255, 255,
  };
  unsigned short out[4] = {0};

  TEST_ASSERT_TRUE(rgba8888ToRgb565(rgba, out, 4));
  TEST_ASSERT_EQUAL_HEX16(0xF800, out[0]);
  TEST_ASSERT_EQUAL_HEX16(0x07E0, out[1]);
  TEST_ASSERT_EQUAL_HEX16(0x001F, out[2]);
  TEST_ASSERT_EQUAL_HEX16(0xFFFF, out[3]);
...
void test_rgb565_buffer_converters_reject_null(void) {
  ...
  TEST_ASSERT_FALSE(rgb888ToRgb565(NULL, out, 1));
  TEST_ASSERT_FALSE(rgb888ToRgb565(rgb, NULL, 1));
  TEST_ASSERT_FALSE(rgba8888ToRgb565(NULL, out, 1));
  TEST_ASSERT_FALSE(rgba8888ToRgb565(rgb, NULL, 1));
}
...
void test_startsWith_exact(void) { TEST_ASSERT_TRUE(startsWith("abc", "abc")); }
...
void test_from_hex_digit(void) { TEST_ASSERT_EQUAL_INT(9, from_hex('9')); }
...
int main(void) {
  ...
  RUN_TEST(test_rgb888ToRgb565_buffer);
  RUN_TEST(test_rgba8888ToRgb565_buffer_ignores_alpha);
