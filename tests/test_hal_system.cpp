...
void test_get_current_architecture_rejects_null_output(void) {
  TEST_ASSERT_EQUAL_INT(HAL_EINVAL, hal_system_get_current_architecture(NULL));
}

void test_get_current_architecture_reports_mock_snapshot(void) {
  hal_mock_set_free_heap(123456u);

  hal_system_architecture_t arch = {};
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_system_get_current_architecture(&arch));
...
void test_get_device_uid_reflects_injected_value(void) {
  const uint8_t custom[HAL_DEVICE_UID_BYTES] = {0xDE, 0xAD, 0xBE, 0xEF,
                                                0xCA, 0xFE, 0xBA, 0xBE};
...
void test_get_device_uid_hex_formats_injected_value(void) {
  const uint8_t custom[HAL_DEVICE_UID_BYTES] = {0x01, 0x23, 0x45, 0x67,
                                                0x89, 0xAB, 0xCD, 0xEF};
  hal_mock_set_device_uid(custom);
