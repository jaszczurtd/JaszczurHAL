...
void test_write_reports_invalid_and_uninitialized_channel(void) {
  hal_mock_dac_reset();
  TEST_ASSERT_EQUAL_INT(HAL_EUNINIT, hal_dac_write(0, 1234));
  TEST_ASSERT_EQUAL_UINT16(0, hal_mock_dac_get(0));
  TEST_ASSERT_EQUAL_INT(HAL_EINVAL, hal_dac_write(2, 1234));
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_dac_init_ex(0));
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_dac_write(0, 1234));
  TEST_ASSERT_EQUAL_UINT16(1234, hal_mock_dac_get(0));
}

void test_write_and_read_back(void) {
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_dac_write(0, 1234));
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_dac_write(1, 4000));
  TEST_ASSERT_EQUAL_UINT16(1234, hal_mock_dac_get(0));
...
void test_write_clamps_to_max(void) {
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_dac_write(0, 9999));
  TEST_ASSERT_EQUAL_UINT16(4095, hal_mock_dac_get(0));
}

void test_write_millivolts(void) {
  TEST_ASSERT_EQUAL_INT(
      HAL_OK, hal_dac_write_millivolts(0, 3300)); // full scale (VREF default)
  TEST_ASSERT_EQUAL_UINT16(4095, hal_mock_dac_get(0));
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_dac_write_millivolts(0, 0));
  TEST_ASSERT_EQUAL_UINT16(0, hal_mock_dac_get(0));
...
void test_write_millivolts_reports_invalid_and_uninitialized_channel(void) {
  hal_mock_dac_reset();
  TEST_ASSERT_EQUAL_INT(HAL_EUNINIT, hal_dac_write_millivolts(0, 1650));
  TEST_ASSERT_EQUAL_INT(HAL_EINVAL, hal_dac_write_millivolts(2, 1650));
  TEST_ASSERT_EQUAL_UINT16(0, hal_mock_dac_get(0));
}

void test_millivolts_above_vref_clamped(void) {
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_dac_write_millivolts(0, 5000));
  TEST_ASSERT_EQUAL_UINT16(4095, hal_mock_dac_get(0));

  TEST_ASSERT_EQUAL_INT(
      HAL_OK, hal_dac_write_millivolts(0, 5000)); // above VREF -> full scale
  TEST_ASSERT_EQUAL_UINT16(4095, hal_mock_dac_get(0));
}
...
int main(void) {
  ...
  RUN_TEST(test_write_reports_invalid_and_uninitialized_channel);
  RUN_TEST(test_write_and_read_back);
  RUN_TEST(test_write_clamps_to_max);
  RUN_TEST(test_write_millivolts);
  RUN_TEST(test_write_millivolts_reports_invalid_and_uninitialized_channel);
