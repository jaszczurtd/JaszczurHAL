 ...
 * - Standard 11-bit ID uses SIDH[10:3] and SIDL[2:0] -> bits [2:0] shifted to [7:5]
...
void test_read_can_message_decodes_id_rtr_dlc_and_payload(void) {
    ...
    /* MCP2515 datasheet framing: ID bytes in SIDH/SIDL/EID8/EID0, RTR in RXBnCTRL bit3,
     * DLC in RXBnDLC[3:0], data in RXBnDm. */
    const uint8_t rx_script[] = {
...
void test_read_can_message_standard_remote_uses_srr_when_ide_zero(void) {
    ...
    const uint8_t rx_script[] = {
        0x00u, 0x00u, 0xFBu, 0xF0u, 0x00u, 0x00u, /* read_id: standard 0x7DF, SRR=1 */
...
void test_write_can_message_sets_rtr_bit_and_keeps_dlc_low_nibble(void) {
    ...
    INT8U payload[MAX_CHAR_IN_MESSAGE] = {0x11u, 0x22u, 0x33u, 0x44u, 0x55u, 0x66u, 0x77u, 0x88u};
    TEST_ASSERT_EQUAL_UINT8(MCP2515_OK, can.setMsg(0x123u, 1u, 0u, 3u, payload));

    can.mcp2515_write_canMsg((INT8U)(MCP_TXB0CTRL + 1u));

    const uint8_t expected[] = {
        MCP_WRITE, (uint8_t)(MCP_TXB0CTRL + 6u), 0x11u, 0x22u, 0x33u,
        MCP_WRITE, (uint8_t)(MCP_TXB0CTRL + 5u), (uint8_t)(0x03u | MCP_RTR_MASK),
        MCP_WRITE, (uint8_t)(MCP_TXB0CTRL + 1u), 0x24u, 0x60u, 0x00u, 0x00u,
    };
    assert_spi_tx_equals(expected, sizeof(expected));
}
...
void test_set_msg_clamps_dlc_to_8_and_write_does_not_overflow_payload(void) {
    ...
    INT8U payload[10] = {0x10u, 0x11u, 0x12u, 0x13u, 0x14u, 0x15u, 0x16u, 0x17u, 0x18u, 0x19u};
    TEST_ASSERT_EQUAL_UINT8(MCP2515_OK, can.setMsg(0x321u, 0u, 0u, 0x0Fu, payload));
    TEST_ASSERT_EQUAL_UINT8(8u, can.m_nDlc);

    can.mcp2515_write_canMsg((INT8U)(MCP_TXB0CTRL + 1u));

    const uint8_t expected[] = {
        MCP_WRITE, (uint8_t)(MCP_TXB0CTRL + 6u), 0x10u, 0x11u, 0x12u, 0x13u, 0x14u, 0x15u, 0x16u, 0x17u,
        MCP_WRITE, (uint8_t)(MCP_TXB0CTRL + 5u), 0x08u,
        MCP_WRITE, (uint8_t)(MCP_TXB0CTRL + 1u), 0x64u, 0x20u, 0x00u, 0x00u,
    };
    assert_spi_tx_equals(expected, sizeof(expected));
}
