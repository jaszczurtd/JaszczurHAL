...
void test_probe_rejects_null(void) { TEST_ASSERT_FALSE(pcf8563_probe(NULL)); }
...
void test_get_datetime_century_bit_set_reads_1900s(void) {
  const uint8_t regs[] = {
      ...
      0x02u, /* weekday 2 */
      (uint8_t)(0x12u |
                PCF8563_CENTURY_BIT), /* month 12, century set -> 19xx */
      0x85u,                          /* year 85 */
  };
  hal_mock_i2c_inject_rx(regs, (int)sizeof(regs));
...
void test_clkout_set_and_get_modes_cover_all_enums(void) {
  struct {
    ...
    uint8_t reg;
      ...
      {PCF8563_CLKOUT_DISABLED, 0x00u}, {PCF8563_CLKOUT_1_HZ, 0x83u},
      {PCF8563_CLKOUT_32_HZ, 0x82u},    {PCF8563_CLKOUT_1024_HZ, 0x81u},
...
void test_get_clkout_mode_rejects_invalid_encoding(void) {
  const uint8_t invalid =
      0x01u; /* FE=0 with nonzero FD pattern is invalid for this decoder */
...
void test_timer_set_and_get_cover_all_modes(void) {
  struct {
    ...
    uint8_t reg;
      ...
      {PCF8563_TIMER_DISABLED, 0x03u}, {PCF8563_TIMER_1_60_HZ, 0x83u},
      {PCF8563_TIMER_1_HZ, 0x82u},     {PCF8563_TIMER_64_HZ, 0x81u},
...
void test_set_timer_rejects_invalid_mode(void) {
  TEST_ASSERT_FALSE(
      pcf8563_set_timer(&s_dev, (pcf8563_timer_clock_t)0x99u, 0x10u));
  TEST_ASSERT_EQUAL_INT(0, hal_mock_i2c_get_write_frame_count());
}
