...
void setUp(void) { s_bus = hal_onewire_init(6); }
...
void test_read_write_and_bit_operations(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32(
      3u, (uint32_t)hal_onewire_write_bytes(s_bus, write_payload, 3u, false));
  ...
  TEST_ASSERT_EQUAL_UINT32(2u,
                           (uint32_t)hal_onewire_read_bytes(s_bus, out, 2u));
...
void test_thread_safety_for_concurrent_calls(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32((uint32_t)(kThreads * kIterations),
                           hal_mock_onewire_get_reset_count(s_bus));
  TEST_ASSERT_EQUAL_UINT32((uint32_t)(kThreads * kIterations),
                           hal_mock_onewire_get_skip_count(s_bus));
