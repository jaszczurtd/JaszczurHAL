...
static void script_push(const char *tx_match, const char *reply) {
    if (s_script_count >= SCRIPT_MAX) return;
...
static void on_tx(hal_uart_t h, const char *text, void *user) {
    (void)user;
    s_tx_count++;
    if (s_script_pos >= s_script_count) return;
...
void test_get_network_time_iso8601_basic(void) {
    ...
    char out[40];
    TEST_ASSERT_EQUAL(HAL_SIMCOM_A76XX_OK,
                      hal_simcom_a76xx_get_network_time_iso8601(s_modem,
                                                                out, sizeof(out)));
...
static void script_mqtt_connect_plain(int ci) {
    char buf[32];
    snprintf(buf, sizeof(buf), "AT+CMQTTDISC=%d", ci); script_push(buf, "\r\nOK\r\n");
    snprintf(buf, sizeof(buf), "AT+CMQTTREL=%d",  ci); script_push(buf, "\r\nOK\r\n");
    script_push("AT+CMQTTSTOP",  "\r\nOK\r\n");
    script_push("AT+CMQTTSTART", "\r\n+CMQTTSTART: 0\r\n\r\nOK\r\n");
    snprintf(buf, sizeof(buf), "AT+CMQTTACCQ=%d", ci); script_push(buf, "\r\nOK\r\n");
    char expected[32];
...
void test_mqtt_connect_invalid_args(void) {
    ...
    hal_simcom_a76xx_mqtt_config_t cfg = {0};
    cfg.broker_host = "h"; cfg.client_id = "c"; cfg.client_index = 5;
    TEST_ASSERT_EQUAL(HAL_SIMCOM_A76XX_INVALID_ARG,
                      hal_simcom_a76xx_mqtt_connect(s_modem, &cfg));
}
...
void test_mqtt_connect_start_fail(void) {
    char buf[32];
    snprintf(buf, sizeof(buf), "AT+CMQTTDISC=%d", 0); script_push(buf, "\r\nOK\r\n");
    snprintf(buf, sizeof(buf), "AT+CMQTTREL=%d",  0); script_push(buf, "\r\nOK\r\n");
    script_push("AT+CMQTTSTOP",  "\r\nOK\r\n");
    script_push("AT+CMQTTSTART", "\r\nERROR\r\n");

    hal_simcom_a76xx_mqtt_config_t cfg = {0};
    cfg.broker_host = "h"; cfg.broker_port = 1883;
    cfg.client_id = "c"; cfg.client_index = 0;
    TEST_ASSERT_EQUAL(HAL_SIMCOM_A76XX_ERROR,
                      hal_simcom_a76xx_mqtt_connect(s_modem, &cfg));
...
void test_mqtt_publish_happy_path(void) {
    ...
    TEST_ASSERT_EQUAL(HAL_SIMCOM_A76XX_OK,
                      hal_simcom_a76xx_mqtt_publish(s_modem, 0,
                                                    "test/topic",
                                                    "hello", 5, 1));
}
...
static void on_msg(int client_index,
                   const char *topic,
    ...
    size_t n = payload_len < sizeof(s_msg_payload) - 1
               ? payload_len : sizeof(s_msg_payload) - 1;
    if (payload && n > 0) memcpy(s_msg_payload, payload, n);
    s_msg_payload[n] = '\0';
...
void test_mqtt_subscribe_happy_path(void) {
    ...
    TEST_ASSERT_EQUAL(HAL_SIMCOM_A76XX_OK,
                      hal_simcom_a76xx_mqtt_subscribe(s_modem, 0,
                                                      "test/topic", 1));
}
...
void test_mqtt_unsubscribe_happy_path(void) {
    ...
    TEST_ASSERT_EQUAL(HAL_SIMCOM_A76XX_OK,
                      hal_simcom_a76xx_mqtt_unsubscribe(s_modem, 0,
                                                        "test/topic"));
}
...
void test_mqtt_rx_reassembly_dispatches_message(void) {
    ...
    s_msg_payload[0] = '\0';
    TEST_ASSERT_EQUAL(HAL_SIMCOM_A76XX_OK,
                      hal_simcom_a76xx_mqtt_set_message_callback(s_modem,
                                                                 on_msg, NULL));
