...
struct packet_sink_t {
  jh_ble_hci_transport_t *transport;
  unsigned packets;
  uint8_t last_type;
  uint16_t last_size;
...
jh_ble_controller_t s_controller;
jh_ble_hci_transport_t s_transport;
...
void receive_packet(void *context, uint8_t packet_type, uint8_t *,
  ...
  sink->last_size = size;
  if (sink->try_reentry) {
    sink->reentry_status = jh_ble_hci_transport_service(sink->transport);
  }
}
...
void queue_frame(uint8_t packet_type, const uint8_t *payload,
  ...
  TEST_ASSERT_LESS_OR_EQUAL_UINT32(
      kFrameCapacity - JH_BLE_HCI_FRAME_HEADER_SIZE, payload_size);
  const size_t index = s_fake.frame_count++;
  s_fake.frames[index][3] = packet_type;
  if (payload_size != 0u) {
    std::memcpy(&s_fake.frames[index][JH_BLE_HCI_FRAME_HEADER_SIZE], payload,
                payload_size);
  }
  s_fake.lengths[index] = JH_BLE_HCI_FRAME_HEADER_SIZE + payload_size;
}

jh_ble_hci_transport_snapshot_t snapshot(void) {
  jh_ble_hci_transport_snapshot_t result{};
  jh_ble_hci_transport_snapshot(&s_transport, &result);
  return result;
}
...
void setUp(void) {
  ...
  TEST_ASSERT_EQUAL_INT(HAL_OK,
                        jh_ble_hci_transport_init(&s_transport, &s_controller,
...
void test_open_uses_factory_wifi_address_plus_one(void) {
  TEST_ASSERT_EQUAL_INT(HAL_OK, jh_ble_hci_transport_open(&s_transport));
  uint8_t address[6]{};
  TEST_ASSERT_EQUAL_INT(HAL_OK,
                        jh_ble_hci_transport_address(&s_transport, address));
  ...
  TEST_ASSERT_EQUAL_INT(HAL_EBUSY, jh_ble_hci_transport_open(&s_transport));
}

void test_open_propagates_controller_initialization_failure(void) {
  s_fake.init_status = HAL_EIO;
  TEST_ASSERT_EQUAL_INT(HAL_EIO, jh_ble_hci_transport_open(&s_transport));
  uint8_t address[6]{};
...
void test_receive_drain_is_bounded_and_resumes_next_service(void) {
  ...
  TEST_ASSERT_EQUAL_INT(HAL_OK, jh_ble_hci_transport_open(&s_transport));
  TEST_ASSERT_EQUAL_INT(HAL_OK, jh_ble_hci_transport_service(&s_transport));
  TEST_ASSERT_EQUAL_UINT(JH_BLE_HCI_SERVICE_BUDGET, s_sink.packets);
  TEST_ASSERT_EQUAL_UINT(JH_BLE_HCI_SERVICE_BUDGET, s_fake.read_calls);
...
void test_malformed_and_failed_reads_propagate_hal_status(void) {
  ...
  s_fake.lengths[0] = JH_BLE_HCI_FRAME_HEADER_SIZE - 1u;
  TEST_ASSERT_EQUAL_INT(HAL_OK, jh_ble_hci_transport_open(&s_transport));
  TEST_ASSERT_EQUAL_INT(HAL_EPROTO, jh_ble_hci_transport_service(&s_transport));
  ...
  TEST_ASSERT_EQUAL_INT(HAL_ETIMEOUT,
                        jh_ble_hci_transport_service(&s_transport));
...
void test_send_prepends_type_and_counts_only_successful_packets(void) {
  ...
  uint8_t *const packet = &frame[JH_BLE_HCI_FRAME_HEADER_SIZE];
  packet[0] = 0x01u;
  packet[1] = 0x02u;
  TEST_ASSERT_EQUAL_INT(
      HAL_OK, jh_ble_hci_transport_send(&s_transport, 0x01u, packet, 2u));
  ...
  TEST_ASSERT_EQUAL_INT(
      HAL_EBUSY, jh_ble_hci_transport_send(&s_transport, 0x02u, packet, 2u));
...
void test_invalidation_closes_transport_and_preserves_failure(void) {
  TEST_ASSERT_EQUAL_INT(HAL_OK, jh_ble_hci_transport_open(&s_transport));
  jh_ble_hci_transport_invalidate(&s_transport, HAL_EHW);
  TEST_ASSERT_EQUAL_INT(HAL_EUNINIT,
                        jh_ble_hci_transport_service(&s_transport));
