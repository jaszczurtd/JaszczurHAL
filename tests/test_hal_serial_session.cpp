...
static const hal_serial_session_vocabulary_t k_test_sc_vocab = {
    ...
    .reply_auth_failed_entropy = "SC_AUTH_FAILED entropy",
};

static void init_session_with_test_vocab(hal_serial_session_t *s,
                                         const char *tag, const char *fw,
                                         const char *build) {
  hal_serial_session_init_with_vocabulary(s, tag, fw, build, &k_test_sc_vocab);
}
...
void test_frame_crc8_matches_reference_vector(void) {
  TEST_ASSERT_EQUAL_UINT8(
      0xF4u, hal_serial_frame_crc8((const uint8_t *)"123456789", 9u));
}
...
void test_session_init_clears_state_and_binds_identity(void) {
  ...
  TEST_ASSERT_EQUAL_STRING("E661A4D1234567AB",
                           s.uid_hex); /* default mock UID */
...
void test_poll_hello_reports_full_identity_and_seq_echo(void) {
  ...
  TEST_ASSERT_EQUAL_UINT32((((uint32_t)1u << 20) ^ 1234u),
...
void test_poll_uses_injected_mock_uid(void) {
  const uint8_t custom_uid[HAL_DEVICE_UID_BYTES] = {0x01, 0x02, 0x03, 0x04,
                                                    0x05, 0x06, 0x07, 0x08};
  uint16_t reply_seq = 0u;
  char reply_payload[192];
...
void test_poll_unknown_payload_with_handler_dispatches_inner_and_reply_seq(
    void) {
  hal_serial_session_t s;
  int user_tag = 17;
  uint16_t reply_seq = 0u;
...
void test_poll_handles_multiple_framed_commands_in_one_rx_buffer(void) {
  ...
  init_session_with_test_vocab(&s, "ECU", "1.0.0", "dev");
  TEST_ASSERT_TRUE(
      hal_serial_frame_encode(10u, "HELLO", frame_a, sizeof(frame_a), &len_a));
  TEST_ASSERT_TRUE(
      hal_serial_frame_encode(11u, "HELLO", frame_b, sizeof(frame_b), &len_b));
 ...
 */
static bool extract_challenge_from_reply(
    uint16_t expected_seq, uint8_t out_challenge[HAL_SC_AUTH_CHALLENGE_BYTES]) {
  uint16_t seq = 0u;
  char payload[192];
...
void test_auth_begin_is_reproducible_for_a_fixed_mock_seed(void) {
  hal_serial_session_t first;
  hal_serial_session_t second;
  uint8_t first_challenge[HAL_SC_AUTH_CHALLENGE_BYTES];
  uint8_t second_challenge[HAL_SC_AUTH_CHALLENGE_BYTES];

  init_session_with_test_vocab(&first, "ECU", "1.0.0", "dev");
  inject_framed_line(1u, "HELLO", '\n');
  hal_serial_session_poll(&first);
  inject_framed_line(2u, "SC_AUTH_BEGIN", '\n');
  hal_serial_session_poll(&first);
  TEST_ASSERT_TRUE(extract_challenge_from_reply(2u, first_challenge));

  hal_mock_secure_random_set_seed(0x0123456789ABCDEFull);
  init_session_with_test_vocab(&second, "ECU", "1.0.0", "dev");
  inject_framed_line(3u, "HELLO", '\n');
  hal_serial_session_poll(&second);
  inject_framed_line(4u, "SC_AUTH_BEGIN", '\n');
  hal_serial_session_poll(&second);
  TEST_ASSERT_TRUE(extract_challenge_from_reply(4u, second_challenge));

  TEST_ASSERT_EQUAL_UINT8_ARRAY(first_challenge, second_challenge,
                                sizeof(first_challenge));
}

void test_auth_begin_without_entropy_fails_closed(void) {
  hal_serial_session_t s;
  uint16_t reply_seq = 0u;
  char reply_payload[128];
  const uint8_t nonzero[HAL_SC_AUTH_CHALLENGE_BYTES] = {
      1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u, 1u,
  };
  const uint8_t zero[HAL_SC_AUTH_CHALLENGE_BYTES] = {0u};
  ...
  memcpy(s.challenge, nonzero, sizeof(s.challenge));
  ...
  TEST_ASSERT_FALSE(s.challenge_pending);
  TEST_ASSERT_EQUAL_UINT8_ARRAY(zero, s.challenge, sizeof(s.challenge));
  TEST_ASSERT_TRUE(decode_last_framed_reply(&reply_seq, reply_payload,
                                            sizeof(reply_payload)));
  TEST_ASSERT_EQUAL_UINT16(2u, reply_seq);
  TEST_ASSERT_EQUAL_STRING("SC_AUTH_FAILED entropy", reply_payload);
}
...
void test_auth_prove_with_correct_mac_authenticates_session(void) {
  ...
  TEST_ASSERT_TRUE(hal_sc_auth_compute_response(
      key, challenge, sizeof(challenge), session_id, mac));
  ...
  TEST_ASSERT_FALSE(s.challenge_pending);
  const uint8_t cleared_challenge[HAL_SC_AUTH_CHALLENGE_BYTES] = {0u};
  TEST_ASSERT_EQUAL_UINT8_ARRAY(cleared_challenge, s.challenge,
                                sizeof(s.challenge));
...
static uint32_t authenticate_session(hal_serial_session_t *s,
                                     uint16_t hello_seq, uint16_t begin_seq,
                                     uint16_t prove_seq) {
...
int main(void) {
  ...
  RUN_TEST(
      test_poll_unknown_payload_with_handler_dispatches_inner_and_reply_seq);
  ...
  RUN_TEST(test_auth_begin_is_reproducible_for_a_fixed_mock_seed);
  RUN_TEST(test_auth_begin_without_entropy_fails_closed);
