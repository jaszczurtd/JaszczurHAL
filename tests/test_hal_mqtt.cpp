...
static void on_message(const char *topic, const uint8_t *payload, uint16_t length, void *user) {
    (void)user;

    s_callback_called = true;
    snprintf(s_callback_topic, sizeof(s_callback_topic), "%s", topic ? topic : "");
...
void test_server_connect_publish_subscribe_and_disconnect(void) {
    ...
    TEST_ASSERT_TRUE(hal_mqtt_publish("telemetry/raw", payload, sizeof(payload), true));
    TEST_ASSERT_EQUAL_STRING("telemetry/raw", hal_mock_mqtt_get_last_publish_topic());
    TEST_ASSERT_EQUAL_UINT16((uint16_t)sizeof(payload), hal_mock_mqtt_get_last_publish_len());
    TEST_ASSERT_EQUAL_UINT8_ARRAY(payload,
                                  hal_mock_mqtt_get_last_publish_payload(),
                                  sizeof(payload));
    TEST_ASSERT_TRUE(hal_mock_mqtt_get_last_publish_retained());

    TEST_ASSERT_TRUE(hal_mqtt_publish_str("telemetry/text", "hello", false));
    TEST_ASSERT_EQUAL_STRING("telemetry/text", hal_mock_mqtt_get_last_publish_topic());
