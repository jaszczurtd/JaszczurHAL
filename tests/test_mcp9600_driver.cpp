...
void test_status_flags_map_to_header_bit_masks(void) {
    ...
    const uint8_t status_rx[] = {
        (uint8_t)(HAL_MCP9600_STATUS_BURST |
                  HAL_MCP9600_STATUS_THUPDATE |
                  HAL_MCP9600_STATUS_INPUTRANGE |
                  HAL_MCP9600_STATUS_ALERT1 |
...
void test_set_alert_temperature_negative_value_is_encoded_twos_complement(void) {
    TEST_ASSERT_TRUE(init_with_id(0x40u));
    hal_mock_i2c_reset_write_log();
...
int main(void) {
    ...
    RUN_TEST(test_set_alert_temperature_negative_value_is_encoded_twos_complement);
