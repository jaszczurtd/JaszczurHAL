...
void test_pool_alarm_fires_after_advance(void) {
  ...
  hal_alarm_id_t id =
      hal_timer_pool_add_alarm_us(pool, 1000, on_alarm, nullptr, false);
...
void test_pool_cancel_is_pool_scoped(void) {
  ...
  hal_alarm_id_t id_a =
      hal_timer_pool_add_alarm_us(pool_a, 1000, on_alarm, nullptr, false);
  hal_alarm_id_t id_b =
      hal_timer_pool_add_alarm_us(pool_b, 1000, on_alarm, nullptr, false);
...
void test_add_alarm_ex_reports_invalid_arg(void) {
  ...
  hal_alarm_id_t id =
      hal_timer_add_alarm_us_ex(1000, nullptr, nullptr, false, &res);
...
void test_pool_add_alarm_ex_reports_pool_full(void) {
  ...
  hal_alarm_id_t id1 = hal_timer_pool_add_alarm_us_ex(pool, 1000, on_alarm,
                                                      nullptr, false, &res1);
  hal_alarm_id_t id2 = hal_timer_pool_add_alarm_us_ex(pool, 1000, on_alarm,
                                                      nullptr, false, &res2);
...
void test_managed_timer_stop_prevents_fire(void) {
  ...
  TEST_ASSERT_EQUAL_INT(HAL_TIMER_OK,
                        hal_timer_create(HAL_TIMER_POOL_DEFAULT, 100, false,
                                         on_managed_timer, nullptr, &timer));
...
void test_managed_periodic_pause_and_resume(void) {
  ...
  TEST_ASSERT_EQUAL_INT(HAL_TIMER_OK,
                        hal_timer_get_remaining_us(timer, &rem_before_pause));
  ...
  TEST_ASSERT_EQUAL_INT(HAL_TIMER_OK,
                        hal_timer_get_remaining_us(timer, &rem_paused));
...
void test_managed_set_period_with_restart(void) {
  hal_timer_t timer = nullptr;
  TEST_ASSERT_EQUAL_INT(HAL_TIMER_OK,
                        hal_timer_create(HAL_TIMER_POOL_DEFAULT, 200, true,
                                         on_managed_timer, nullptr, &timer));
  TEST_ASSERT_NOT_NULL(timer);
...
void test_managed_get_remaining_stopped_returns_error(void) {
  ...
  int64_t rem = -1;
  TEST_ASSERT_EQUAL_INT(HAL_TIMER_ERR_NOT_RUNNING,
                        hal_timer_get_remaining_us(timer, &rem));
  TEST_ASSERT_EQUAL_INT(HAL_TIMER_OK, hal_timer_destroy(timer));
}
