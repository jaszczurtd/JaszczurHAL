...
#if (defined(HAL_TARGET_IS_MOCK) && HAL_TARGET_IS_MOCK) && defined(HAL_ENABLE_ONEWIRE)
...
void test_write_bit_one_leaves_line_high(void) {
    ...
    TEST_ASSERT_TRUE_MESSAGE(hal_mock_gpio_get_state(OW_PIN),
        "write_bit(1) left the line LOW: onewire_drive_high drove before set_mode(OUTPUT)");
    TEST_ASSERT_TRUE(hal_mock_gpio_is_output(OW_PIN));
}

/* write_bit(0) is a 65us low slot, also released HIGH at the end via drive_high. */
void test_write_bit_zero_leaves_line_high(void) {
    JHOneWire ow(OW_PIN);
    ow.write_bit(0u);
    TEST_ASSERT_TRUE_MESSAGE(hal_mock_gpio_get_state(OW_PIN),
        "write_bit(0) left the line LOW: onewire_drive_high drove before set_mode(OUTPUT)");
}
