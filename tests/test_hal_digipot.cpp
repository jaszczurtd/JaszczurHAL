...
void test_mcp401x_init_ex_reports_status_and_sets_handle(void) {
  hal_digipot_config_t cfg = {};
  cfg.chip = HAL_DIGIPOT_CHIP_MCP401X;
  cfg.i2c_bus = 0;
  cfg.e2e_resistance = 10000;
  ...
  TEST_ASSERT_EQUAL_INT(HAL_OK, hal_digipot_init_ex(&cfg, &h));
  TEST_ASSERT_NOT_NULL(h);
  hal_digipot_deinit(h);

  h = (hal_digipot_t)0x1;
  cfg.e2e_resistance = 7000;
  TEST_ASSERT_EQUAL_INT(HAL_EINVAL, hal_digipot_init_ex(&cfg, &h));
  TEST_ASSERT_NULL(h);

  cfg.e2e_resistance = 10000;
  TEST_ASSERT_EQUAL_INT(HAL_EINVAL, hal_digipot_init_ex(&cfg, NULL));
  TEST_ASSERT_EQUAL_INT(HAL_EINVAL, hal_digipot_init_ex(NULL, &h));
  TEST_ASSERT_NULL(h);
}
...
void test_mcp401x_set_resistance_wl(void) {
  ...
  hal_mock_i2c_inject_rx(&wrong, 1);
  TEST_ASSERT_EQUAL_INT(HAL_EIO, hal_digipot_set_resistance_ex(h, 5000));
...
void test_mcp401x_wh_mode_on_mcp4018_inverts_wiper_and_uses_fixed_address(
    void) {
  hal_digipot_config_t cfg = {};
  cfg.chip = HAL_DIGIPOT_CHIP_MCP401X;
  cfg.i2c_bus = 0;
...
void test_mcp401x_readback_failure_returns_false(void) {
  ...
  hal_mock_i2c_set_busy(
      true); /* request_from/read fails via busy backend path */
  TEST_ASSERT_FALSE(hal_digipot_set_resistance(h, 5000));
  TEST_ASSERT_EQUAL_INT(HAL_EBUS, hal_digipot_set_resistance_ex(h, 5000));
  hal_mock_i2c_set_busy(false);
...
void test_mcp401x_resistance_above_e2e_rejected(void) {
  ...
  TEST_ASSERT_EQUAL_INT(HAL_EINVAL, hal_digipot_set_resistance_ex(h, 20000));
...
void test_max5395_init_ex_reports_i2c_failure(void) {
  hal_digipot_config_t cfg = {};
  cfg.chip = HAL_DIGIPOT_CHIP_MAX5395;
  cfg.i2c_addr = 0x28;
  cfg.e2e_resistance = 10000;
  ...
  hal_digipot_t h = NULL;
  TEST_ASSERT_EQUAL_INT(HAL_EBUS, hal_digipot_init_ex(&cfg, &h));
  TEST_ASSERT_NULL(h);
  hal_mock_i2c_set_busy(false);
}
...
void test_public_api_guards_and_pool_limit(void) {
  ...
  TEST_ASSERT_EQUAL_INT(HAL_EUNINIT, hal_digipot_set_resistance_ex(NULL, 1000));
  TEST_ASSERT_EQUAL_UINT16(0u, hal_digipot_step_count(NULL));
  TEST_ASSERT_EQUAL_UINT32(0u, hal_digipot_e2e_resistance(NULL));
  TEST_ASSERT_EQUAL_INT(HAL_DIGIPOT_MODE_VOLTAGE_DIVIDER,
                        hal_digipot_mode(NULL));
  ...
  TEST_ASSERT_NULL(hal_digipot_init(&cfg));
  hal_digipot_t extra = NULL;
  TEST_ASSERT_EQUAL_INT(HAL_ENOMEM, hal_digipot_init_ex(&cfg, &extra));
...
int main(void) {
  ...
  RUN_TEST(test_mcp401x_init_ex_reports_status_and_sets_handle);
  RUN_TEST(test_mcp401x_set_resistance_wl);
  RUN_TEST(test_mcp401x_set_resistance_voltage_divider);
  RUN_TEST(
      test_mcp401x_wh_mode_on_mcp4018_inverts_wiper_and_uses_fixed_address);
  ...
  RUN_TEST(test_max5395_init_ex_reports_i2c_failure);
