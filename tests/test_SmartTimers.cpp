...
/* Rule of Three: SmartTimers owns a hal_mutex_t freed in its destructor, so it
   must be non-copyable - a copy would share the handle and double-free it.
   These compile-time guards fail the build if the deleted copy ops regress. */
static_assert(
    !std::is_copy_constructible<SmartTimers>::value,
    "SmartTimers must not be copy-constructible (double-free of _mutex)");
static_assert(
    !std::is_copy_assignable<SmartTimers>::value,
    "SmartTimers must not be copy-assignable (double-free of _mutex)");
