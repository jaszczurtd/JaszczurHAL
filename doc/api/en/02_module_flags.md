...
*Also available in [Polish](../pl/02_module_flags.md).*
...
| `HAL_ENABLE_BLUETOOTH_CLASSIC` | `hal_bluetooth_classic.h` | shared manager, bond codec/provider + target backend | Nonblocking Classic inquiry, copied results, SDP, bounded incoming pairing and indexed peer storage (propagates CRC); see the [Bluetooth API](20_bluetooth.md#bluetooth-classic-manager-and-profiles) |
| `HAL_ENABLE_BLUETOOTH_A2DP_SINK` | `hal_bluetooth_a2dp_sink.h` | bounded SBC/PCM runtime + BTstack A2DP Sink and Bluedroid decoder | One incoming SBC stream with signed PCM output, prebuffering, queue diagnostics and shared Classic bonding (propagates BLUETOOTH_CLASSIC) |
| `HAL_ENABLE_BLUETOOTH_AVRCP_TARGET` | `hal_bluetooth_avrcp_target.h` | minimal BTstack AVRCP Target | Absolute-volume receive/report support sharing the A2DP connection and bond (propagates BLUETOOTH_A2DP_SINK) |
| `HAL_ENABLE_BLUETOOTH_HID_HOST` | `hal_bluetooth_hid_host.h` | generic HID profile + target HID support | One raw Classic HID Host connection with copied descriptor and bounded Input/Output/Feature reports (propagates BLUETOOTH_CLASSIC) |
| `HAL_ENABLE_BLUETOOTH_GAMEPAD` | `hal_gamepad.h` | HID gamepad adapter and parser | One nonblocking Classic HID gamepad with explicit pairing/reconnect, normalized copied snapshots and bounded diagnostics (propagates BLUETOOTH_HID_HOST); see the [Bluetooth API](20_bluetooth.md#gamepad-adapter) |
