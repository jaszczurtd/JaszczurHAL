...
[Native Windows Setup](../../en/windows_setup.md) for host requirements, commands,
...
Synchronizes `third_party/BTstack` from `third_party/btstack_version.conf`
through `python3 scripts/component_manager.py component btstack`. Options
mirror the BearSSL helper.

### `scripts/ensure_sx126x.sh`

Synchronizes `third_party/sx126x_driver` from
`third_party/sx126x_driver_version.conf` through
`python3 scripts/component_manager.py component sx126x`. It verifies the
clean exact commit, Clear BSD license, base driver and version source set, and
the HAL/status/register headers. Options mirror the BearSSL helper.
...
[Native OTA Workflow](../../en/OTAWorkflow.md) for the complete consumer workflow
...
Validates the bilingual documentation layout and every English/Polish pair
...
- [JaszczurHAL Library Compilation](../../en/lib_compilation.md) describes static
  ...
  equivalents.
- [Firmware Project Workflow](../../en/FwProjectWorkflow.md) describes
...
- [Target and board profiles](../../en/boards_profiles_howto.md) documents descriptor
...
- [Security Supply Chain](../../en/security_supply_chain.md) documents SBOM generation,
