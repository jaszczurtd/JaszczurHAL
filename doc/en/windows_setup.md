<a id="native-windows-setup"></a>

# Setting up a native Windows environment

*Also available in [Polish](../pl/windows_setup.md).*

The JaszczurHAL setup script prepares a firmware development environment directly on Windows. It installs pinned tools without WSL, Git Bash, winget, Chocolatey, or a globally installed toolchain.

The minimum supported system is Windows 10 1809 (build 17763), AMD64. Install Git for Windows and VS Code before cloning and opening the repository. The setup script manages Python, CMake, Ninja, GNU Arm Embedded, GNU RISC-V, OpenOCD, picotool, pyserial, and pinned source dependencies. On first use, the ESP32-S3 runner also prepares the pinned ESP-IDF revision and its official target tools in `third_party\esp-idf` and `%USERPROFILE%\.espressif`. The GNU Arm check includes GDB. Reusing an OpenOCD installation requires the CMSIS-DAP, ST-Link, RP2040, RP2350, and STM32G4 scripts used by the generated debug configurations.
...
By default, setup checks both settings without changing them. If Git long-path support is missing, it prints the required command. Pass `-ConfigureHost` to permit changes to `core.longpaths`. The script can change the Windows registry setting only from an already elevated PowerShell session; it never elevates itself.
...
Keep the working copy on a local Windows volume, such as `C:`. The script rejects WSL UNC paths (`\\wsl.localhost\...`) because Git for Windows cannot safely manage and update that working copy.
...
Setup records the configuration in `.build\windows\host-environment.json`. Project tools use this file to select verified paths for CMake, Ninja, GNU Arm, GNU RISC-V, OpenOCD, picotool, Python, and the build root. `-VerifyOnly` compares the recorded state byte-for-byte with the configuration resolved by the current check.
...
Cortex-Debug uses the verified paths recorded during setup. To inspect the tools selected for a particular project, run `debug-tools`:
...
Every successful build refreshes the selected target artifacts, even when Ninja has nothing to rebuild. Switching targets therefore cannot leave the previous target image in `buildDir`. Starting a build removes the stable set of uploadable files; a failed configuration or compilation cannot leave an old image available for a later upload.

`clean` checks path safety and then removes both managed directories.
...
To check compilation and linking of the Phase 3 ESP32-S3 test project, run this in native PowerShell:
...
This project checks compilation and linking only. It does not validate uploading or monitoring on hardware. Device projects use `list-ports`, `upload`, and `monitor` with the COM port exposed by the board's USB Serial/JTAG interface.
...
GitHub Actions builds a generated application from a Windows path containing spaces for RP2040, RP2350 ARM, RP2350 RISC-V, and STM32G474. Checks cover Ninja configuration, the static-library CMake target where applicable, representative firmware, declared artifacts, the adjusted compilation database, and MSVC warning settings. They also verify that Windows-incompatible POSIX, FreeRTOS, and BearSSL host tests are explicitly marked as disabled. The MSVC job builds and runs a focused HAL CRC test and checks the portable BSD socket-header interface with `/W4 /permissive- /WX`. The full BSD adapter exports POSIX symbols rather than the Winsock ABI, so its tests run on firmware or a Linux host. BearSSL integration tests also require Linux because their test harness and transport use Bash and POSIX sockets.

The `windows-tooling` job caches the pinned ESP-IDF revision and official tools, performs a clean production build of `tests/fixtures/esp32s3_phase3`, and publishes its relocatable manifest, build log, bootloader, partition table, and application images. A successful CI build does not validate Phase 3 behavior on hardware.

The repository does not define a Windows static-analysis profile. The documented managed tool set and host environment provide neither `clang-tidy` nor `cppcheck`, and MSVC Build Tools are not a version-pinned setup component. Compilation with strict MSVC warnings remains the required Windows check. Add a static-analysis profile only with an authenticated, version-pinned analyzer executable so local and CI results remain comparable.
...
Windows support covers project launching and compilation, generated VS Code task overrides, line-ending handling, component management, and host setup. It also includes the four-family CMake firmware build matrix, COM and BOOTSEL uploads, OTA firewall configuration, debug-tool discovery, socket-header portability checks, production ESP32-S3 ESP-IDF build/upload/monitor operations, and Windows CI.
