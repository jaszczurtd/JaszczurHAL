...
| Static libraries | One runner per build family produces `libJaszczurHAL.a` for RP with the official Pico SDK, for STM32G474, and for ESP32 with ESP-IDF; a single entry script selects the runner from the target registry. The RP build also checks ELF/BIN/UF2 generation and application-entry symbols. | [library runners](../../link_libraries/README.md) |
...
| Area | What it offers | Source |
|---|---|---|
| Coordinated flash access | On RP, a shared coordinator serializes EEPROM/KV, LittleFS, and OTA staging operations. It makes the other core safe, pauses TinyUSB, rejects XIP-resident callbacks and busy DMA that touches flash (peripheral-to-RAM rings keep running), applies timeouts, and restores the previous state afterward. On STM32G474, one flash mutex serializes erase/program sequences for EEPROM/KV and LittleFS. | [RP flash drivers](../../src/hal/impl/rp2040/drivers/flash/), [storage API](../api/en/14_storage.md) |
