...
| Board profiles and runtime capabilities | Generated RP2040, RP2350, STM32G474, ESP32, ESP32-S3, and host profiles from the board registry. Supported builds expose the shared runtime board facade. | [board registry](../../boards/README.md), [hal_board.h](../../src/hal/system/hal_board.h) |
...
| ESP32 family backend | Implements the shared JaszczurHAL API on top of ESP-IDF. ESP32-S3 provides the delivered core, peripheral, network, NimBLE base-BLE and OTA set. Project tooling supplies generated board/memory configuration and build/upload/monitor/IntelliSense integration for both targets. | [ESP32 implementation](../../src/hal/impl/esp32/), [ESP32-S3 compile fixture](../../tests/fixtures/esp32s3_phase3/) |
...
| Dispatcher (vsc project generator)-backed firmware projects | Shared VS Code/CMake workflow for generated projects, migrated downstream modules and checked-in examples, with target/board selection and per-target CMake cache isolation. | [FwProjectWorkflow.md](FwProjectWorkflow.md) |
...
| Validation gate | Full local gate for unit tests, Clang ASan/UBSan/libFuzzer checks, Valgrind, static analysis, RP/STM target builds, the compile-only ESP32-S3 multi-feature, and examples. | [runalltests.sh](../../runalltests.sh) |
...
| I2C master | Portable i2c controller API with two-bus support, atomic helpers, bus recovery, optional 10-bit addressing (`HAL_ENABLE_I2C_10BIT`) and a bounded 7-bit scanner accepting a watchdog/progress callback. | [hal_i2c.h](../../src/hal/i2c/hal_i2c.h) |
...
| WiFi | CYW43/lwIP connectivity on Pico W, Pico 2 W, Pico+PIM730 (CYW43 WiFi module) and STM32G474+PIM730 hardware, plus native ESP-IDF WiFi/`esp_netif`/lwIP on ESP32-S3. | [hal_wifi.h](../../src/hal/network/hal_wifi.h) |
| BLE Peripheral and Observer | One Peripheral connection, copied legacy advertising, passive scanning with bounded copied reports and AD parsing, static GAP/GATT services, ATT MTU reporting, CYW43/BTstack backends, ESP32-S3 NimBLE base-BLE, and a deterministic host mock. | [Bluetooth API](../api/en/20_bluetooth.md) |
| Bluetooth Classic manager | Public inquiry, copied results, pairing, SDP and indexed bonded-peer management shared by opt-in Classic profiles. HID Host adds raw descriptors/reports; gamepad is a separate adapter. Implementations use CYW43/BTstack or Bluedroid on the original ESP32. | [module flags](../api/en/02_module_flags.md), [Bluetooth API](../api/en/20_bluetooth.md) |
| Bluetooth Classic HID | The generic HID Host exposes a single copied descriptor and limited raw Input/Output/Feature reports without making device-specific assumptions. | [hal_bluetooth_hid_host.h](../../src/hal/bluetooth/hal_bluetooth_hid_host.h) |
| Bluetooth gamepad | The optional gamepad adapter on top of Bluetooth Classic that returns normalized buttons, axes, and D-pad state, and clears inputs after disconnect. | [hal_gamepad.h](../../src/hal/bluetooth/hal_gamepad.h) |
| Bluetooth A2DP Sink and AVRCP Target | One bounded SBC stream with signed 16-bit PCM output, optional mono downmix, shared Classic bonding, stream diagnostics, and absolute-volume control. | [Bluetooth API](../api/en/20_bluetooth.md#a2dp-sink-and-avrcp-target), [hal_bluetooth_a2dp_sink.h](../../src/hal/bluetooth/hal_bluetooth_a2dp_sink.h), [hal_bluetooth_avrcp_target.h](../../src/hal/bluetooth/hal_bluetooth_avrcp_target.h) |
...
| BLE Stream commands | Functionality similar to LoRa commands, operating over BLE. It supports fragmentation and reassembly of binary command messages with MTU awareness, automatic routing of requests to the command router and sending back responses, authenticated peer and session metadata, and session closure when its state cannot be safely reconstructed. The adapter takes exclusive ownership of a single authenticated JH BLE Stream session. | [command API](../api/en/23_commands.md#authenticated-ble-stream-adapter), [hal_ble_commands.h](../../src/hal/bluetooth/hal_ble_commands.h) |
| HTTP server | Small poll-driven plaintext HTTP/1.1 server over HAL TCP with exact/prefix routes, request headers, automatic `Content-Length`, resumable partial writes, and bounded response/idle timeouts. No HTTPS-server API is defined. | [hal_http_server.h](../../src/hal/network/http/hal_http_server.h) |
...
| WireGuard | Shared host-lwIP WireGuard integration with split/full tunnel routing. | [hal_wireguard.h](../../src/hal/network/wireguard/hal_wireguard.h), [WireGuard engine](../../src/hal/network/wireguard/core/) |
...
| Key-value storage (K/V) | Persistent two-bank layer based on EEPROM-like storage, with data protection mechanisms for failure scenarios. Provides a status-returning `get` / `set` / `commit` API (`hal_status_t`). | [hal_kv.h](../../src/hal/storage/hal_kv.h) |
