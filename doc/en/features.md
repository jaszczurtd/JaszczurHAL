...
| WiFi | CYW43/lwIP connectivity on Pico W, Pico 2 W, Pico+PIM730 (CYW43 WiFi module) and configured STM32G474+PIM730 hardware, plus native ESP-IDF WiFi/`esp_netif`/lwIP on ESP32-S3. | [hal_wifi.h](../../src/hal/network/hal_wifi.h) |
...
| Bluetooth Classic host foundation | Shared controller, HCI transport, and host lifecycle support for opt-in Classic profiles. `HAL_ENABLE_BLUETOOTH_CLASSIC` is selected by a profile and does not expose a device API by itself. Implementations use CYW43/BTstack or Bluedroid on the original ESP32. | [module flags](../api/en/02_module_flags.md), [Bluetooth API](../api/en/20_bluetooth.md) |
| Bluetooth Classic HID gamepad profile | An optional nonblocking host profile for one gamepad, with bounded pairing, reconnect, and persistent bonding through a provider. `hal_gamepad` returns normalized buttons/axes/D-pad snapshots, reports overflow explicitly, and releases inputs on disconnect. A deterministic host mock is also available. | [hal_gamepad.h](../../src/hal/bluetooth/hal_gamepad.h), [Bluetooth API](../api/en/20_bluetooth.md#bluetooth-classic-hid-gamepad) |
