...
| A | [Status API](api/01_status_api.md) | Foundational, cross-cutting: `hal_status_t` result codes, status helper functions, and the additive status-returning `_ex` naming convention (output-parameter and handle forms, collision fallback). Read before the per-module sections; each module documents its own `_ex` variants inline. |
...
| 9 | [Communication buses](api/09_buses.md) | `hal_spi` (status `_ex` transfer and DMA helpers), `hal_i2c` (master, status `_ex` helpers, one-shot helpers, bus-clear), `hal_i2c_slave` (register map), `hal_uart`, `hal_swserial`, `hal_onewire` |
