...
| Baza hosta Bluetooth Classic | Wspólna obsługa kontrolera, transportu HCI i cyklu życia hosta dla opcjonalnych profili Classic. `HAL_ENABLE_BLUETOOTH_CLASSIC` jest włączane przez wybrany profil i samo nie udostępnia API urządzenia. Implementacje korzystają z CYW43/BTstack albo Bluedroid na oryginalnym ESP32. | [flagi modułów](../api/pl/02_module_flags.md), [API Bluetooth](../api/pl/20_bluetooth.md) |
| Profil gamepada Bluetooth Classic HID | Opcjonalny, nieblokujący profil hosta dla jednego gamepada, z ograniczonym czasowo parowaniem, ponownym łączeniem i trwałym bondingiem przez provider. `hal_gamepad` zwraca znormalizowane struktury stanu przycisków, osi i D-pada, jawnie sygnalizuje przepełnienie oraz zeruje wejścia po rozłączeniu. Dostępna jest także deterministyczna implementacja testowa na hoście. | [hal_gamepad.h](../../src/hal/bluetooth/hal_gamepad.h), [API Bluetooth](../api/pl/20_bluetooth.md#gamepad-bluetooth-classic-hid) |
