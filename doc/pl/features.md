...
| Profile płytek i możliwości runtime | Generowane profile RP2040, RP2350, STM32G474, ESP32, ESP32-S3 i hosta pochodzące z rejestru płytek. Wspierane buildy udostępniają współdzieloną fasadę płytki w runtime. | [rejestr płytek](../../boards/README.md), [hal_board.h](../../src/hal/system/hal_board.h) |
...
| Backend rodziny ESP32 | Implementuje współdzielone API JaszczurHAL na ESP-IDF. ESP32-S3 dostarcza obecny zestaw core, peryferiów, sieci, bazowego BLE NimBLE i OTA. Narzędzia projektowe zapewniają generowaną konfigurację płytki/pamięci oraz integrację build/upload/monitor/IntelliSense dla obu dokładnych targetów. | [implementacja ESP32](../../src/hal/impl/esp32/), [fixture buildu ESP32-S3](../../tests/fixtures/esp32s3_phase3/), [fixture buildu gamepada ESP32](../../tests/fixtures/esp32_gamepad/) |
...
| Projekty firmware oparte na dispatcherze (generatorze projektów vsc) | Wspólny workflow VS Code/CMake dla generowanych projektów, przeniesionych modułów downstream i przykładów w repozytorium, z wyborem targetu/płytki i osobnym cache CMake dla każdego targetu. | [FwProjectWorkflow.md](FwProjectWorkflow.md) |
...
| Bramka walidacji | Pełna lokalna bramka dla testów jednostkowych, kontroli Clang ASan/UBSan/libFuzzer, Valgrind, analizy statycznej, buildów targetów RP/STM, ESP-IDF ESP32-S3, oraz przykładów. | [runalltests.sh](../../runalltests.sh) |
...
| BLE Peripheral i Observer | Jedno połączenie Peripheral, kopiowane klasyczne rozgłaszanie, pasywne skanowanie z ograniczonymi kopiowanymi raportami i parsowaniem AD, statyczne usługi GAP/GATT, raportowanie ATT MTU, backendy CYW43/BTstack, bazowe BLE NimBLE na ESP32-S3 oraz deterministyczny mock hosta. | [API Bluetooth](../api/pl/20_bluetooth.md) |
| Gamepad Bluetooth Classic HID | Jeden nieblokujący profil hosta z ograniczonym parowaniem, reconnect znanego urządzenia w bieżącym runtime, znormalizowanymi snapshotami przycisków/osi/D-pada, jawnym raportowaniem przepełnienia, zerowaniem wejść po rozłączeniu, backendami CYW43/BTstack i Bluedroid na oryginalnym ESP32 oraz deterministycznym mockiem hosta. | [hal_gamepad.h](../../src/hal/bluetooth/hal_gamepad.h), [API Bluetooth](../api/pl/20_bluetooth.md#gamepad-bluetooth-classic-hid) |
...
| Serwer HTTP | Mały, sterowany pollingiem serwer HTTP/1.1 w czystym tekście przez HAL TCP z trasami dokładnymi/prefiksowymi, nagłówkami żądania, buforowanymi odpowiedziami, automatycznym `Content-Length` i obsługą żądań testowalną mockiem. HTTPS dla serwera nie jest jeszcze wspierane. | [hal_http_server.h](../../src/hal/network/http/hal_http_server.h) |
...
| WireGuard | Współdzielona integracja WireGuard z host-lwIP z routingiem split/full tunnel na backendach wspierających tę możliwość. | [hal_wireguard.h](../../src/hal/network/wireguard/hal_wireguard.h), [silnik WireGuard](../../src/hal/network/wireguard/core/) |
...
| Obszar | Co oferuje | Źródło |
|---|---|---|
| Koordynacja mutacji flash | Natywny RP prowadzi EEPROM/KV, LittleFS i etapowanie OTA przez jeden koordynator, który serializuje wywołujących, zabezpiecza drugi rdzeń, wstrzymuje TinyUSB, odrzuca callbacki rezydujące w XIP i aktywne DMA, stosuje ograniczone timeouty oraz przywraca stan runtime. STM32G474 serializuje sekwencje erase/program EEPROM/KV i LittleFS jednym mutexem flash. | [drivery flash RP](../../src/hal/impl/rp2040/drivers/flash/), [API pamięci masowej](../api/pl/14_storage.md) |
...
| Pamięć klucz-wartość (K/V) | Mała trwała warstwa klucz-wartość na bazie pamięci stylu EEPROM, wraz z API get/set/commit zwracającym status (`hal_status_t`). | [hal_kv.h](../../src/hal/storage/hal_kv.h) |
| LittleFS | Jedna thread-safe, niezależna od targetu fasada odpowiada za cykl życia, walidację, stan zamontowania, zapytania o rozmiar i konfigurację postępu nad wspólnym providerem littlefs. Natywny RP i STM32G474 używają zarezerwowanych przez linker partycji wewnętrznej flash; mock wstrzykuje wyniki providera przez tę samą fasadę. | [fasada i provider LittleFS](../../src/hal/storage/), [API pamięci masowej](../api/pl/14_storage.md) |
