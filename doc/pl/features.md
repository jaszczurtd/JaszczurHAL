...
| Polecenia LoRa | Funkcjonalność wysyłania zdefiniowanych przez użytkownika poleceń z automatycznie odsyłanymi odpowiedziami, oparta o powyżej opisane łącze LoRa. Kolejki mają ograniczoną pojemność i przechowują własne kopie danych, a metadane łącza są przekazywane do procedur obsługi. | [API poleceń](../api/pl/23_commands.md), [hal_lora_commands.h](../../src/hal/radio/hal_lora_commands.h) |
| Fasada CAN | Wspólne, konfigurowalne API wybierające w zależności od konfiguracji implementację klasycznego CAN lub CAN FD. | [hal_can.h](../../src/hal/can/hal_can.h) |
...
| WiFi | Łączność CYW43/lwIP na Pico W, Pico 2 W, Pico+PIM730 (moduł WiFi z CYW43) oraz STM32G474+PIM730, plus natywne WiFi/`esp_netif`/lwIP z ESP-IDF na ESP32-S3. | [hal_wifi.h](../../src/hal/network/hal_wifi.h) |
...
| Manager Bluetooth Classic | Publiczne inquiry, kopiowane wyniki, pairing, SDP i indeksowane zarządzanie zapisanymi urządzeniami współdzielone przez opcjonalne profile Classic. HID Host dodaje surowe deskryptory i raporty dla osobnych adapterów. Implementacje korzystają z CYW43/BTstack albo Bluedroid na oryginalnym ESP32. | [flagi modułów](../api/pl/02_module_flags.md), [API Bluetooth](../api/pl/20_bluetooth.md) |
| Bluetooth Classic HID | Ogólny HID Host udostępnia jeden skopiowany deskryptor i ograniczone surowe raporty Input/Output/Feature bez założeń o urządzeniu. | [hal_bluetooth_hid_host.h](../../src/hal/bluetooth/hal_bluetooth_hid_host.h) |
| Bluetooth gamepad | Opcjonalny adapter gamepada, zbudowany jako klient Bluetooth Classic, zwraca znormalizowane przyciski, osie i D-pad oraz zeruje wejścia po rozłączeniu. | [hal_gamepad.h](../../src/hal/bluetooth/hal_gamepad.h) |
| Bluetooth A2DP Sink i AVRCP Target | Jeden strumień SBC z ograniczonymi buforami, wyjściem 16-bitowego PCM ze znakiem, opcjonalnym miksem mono, wspólną obsługą bondów Classic, diagnostyką strumienia i bezwzględną regulacją głośności. | [API Bluetooth](../api/pl/20_bluetooth.md#a2dp-sink-i-avrcp-target), [hal_bluetooth_a2dp_sink.h](../../src/hal/bluetooth/hal_bluetooth_a2dp_sink.h), [hal_bluetooth_avrcp_target.h](../../src/hal/bluetooth/hal_bluetooth_avrcp_target.h) |
...
| Polecenia BLE Stream | Funkcjonalność podobna do poleceń LoRa, działająca w obszarze BLE. Obsługuje fragmentację i ponowne składanie binarnych komunikatów poleceń z uwzględnieniem MTU, automatyczne kierowanie żądań do routera poleceń i odsyłanie odpowiedzi, uwierzytelnione metadane drugiej strony i sesji oraz zamknięcie sesji, gdy nie można bezpiecznie odtworzyć jej stanu. Adapter przejmuje wyłączną obsługę jednej uwierzytelnionej sesji JH BLE Stream. | [API poleceń](../api/pl/23_commands.md#uwierzytelniony-adapter-ble-stream), [hal_ble_commands.h](../../src/hal/bluetooth/hal_ble_commands.h) |
| Serwer HTTP | Mały serwer HTTP/1.1 bez szyfrowania, obsługiwany przez regularne wywołania `poll()`. Udostępnia trasy dokładne i prefiksowe, nagłówki żądania, automatyczny `Content-Length`, wznawianie po częściowym zapisie TCP oraz timeouty odpowiedzi i bezczynności. API serwera nie obsługuje HTTPS. | [hal_http_server.h](../../src/hal/network/http/hal_http_server.h) |
