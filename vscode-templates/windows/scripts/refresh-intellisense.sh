...
#
# Sketch issue: arduino-cli records sketch translation units in .build/sketch.
# That breaks cpptools path matching for files opened from the workspace, so
# we patch compile_commands by adding duplicated entries that point to the
...
for root, dirs, files in os.walk(project_dir):
    if ".build" in root:
        continue
    for name in files:
        project_sources.setdefault(name, os.path.join(root, name))

for entry in original_commands:
    src_file = entry.get("file", "")
    src_norm = os.path.normpath(src_file)
    sketch_dir = os.path.normpath(os.path.join(build_dir, "sketch"))
...
print(f"  compile_commands: {patched_db_path} ({len(patched_commands)} entries, {len(patched_commands) - len(original_commands)} workspace path mappings)")
