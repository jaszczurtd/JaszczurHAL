...
def main():
    ...
    port = settings.get("arduino.uploadPort", "COM3")
    baud = 115200
    ...
    if list_ports is not None:
        available_ports = sorted(p.device for p in list_ports.comports())

    if port not in available_ports and not os.path.exists(port):
        print(f"\033[0;31mPort {port} nie istnieje\033[0m")
        print()
        print("Dostępne porty:")
        import glob
        ports = available_ports or (glob.glob("/dev/ttyACM*") + glob.glob("/dev/ttyUSB*"))
