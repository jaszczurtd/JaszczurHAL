...
generator override. Standalone projects generated by
`tools/create-vscode-example.py` and the checked-in repository examples also
mirror the initial dispatcher cache into `.vscode/settings.json` as
`cmake.configureSettings`, so VS Code CMake Tools can configure the shared
dispatcher directly when a project folder opens. Its isolated
`cmake.buildDirectory` keeps that automatic configuration separate from builds
started through `jh-vscode` tasks.
...
points `cmake.sourceDir` at the shared JaszczurHAL dispatcher, stores the
initial `target`/`board` selection, and settings provide the same initial cache
to CMake Tools for direct editor configuration. Use `--target` and `--board` to
choose a non-default initial board:
