#!/usr/bin/env bash
# Fetch or verify the pinned RISC-V bare-metal GCC for the rp2350-riscv target.
# Prebuilt tarball from the official Raspberry Pi pico-sdk-tools releases, so it
# is managed independently. Shares only logging with the other dependency
# scripts (scripts/lib/pinned_repo.sh); the payload is a tarball, not a git repo.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "${SCRIPT_DIR}/.." && pwd)"
CONFIG_FILE="${REPO_ROOT}/third_party/riscv_toolchain_version.conf"
...
usage() {
    cat <<'USAGE'
Fetch or verify the pinned RISC-V toolchain for the rp2350-riscv target.
...
# shellcheck source=../third_party/riscv_toolchain_version.conf
source "${CONFIG_FILE}"
...
if [[ ${ENABLE} -eq 0 ]]; then
    info "rp2350-riscv target not requested; skipping RISC-V toolchain ensure."
    exit 0
fi
...
STAMP="${TC_DIR}/.jaszczurhal-component-version"
EXPECTED_STAMP="${RISCV_TOOLCHAIN_TAG}|${ASSET}|${RISCV_TOOLCHAIN_TRIPLE}|gcc-${RISCV_TOOLCHAIN_GCC_MAJOR}"
INSTALLED_STAMP=""
[[ -f "${STAMP}" ]] && INSTALLED_STAMP="$(<"${STAMP}")"
...
if [[ ${VERIFY_ONLY} -eq 1 ]]; then
    ...
    [[ "${INSTALLED_STAMP}" == "${EXPECTED_STAMP}" ]] ||
        die "RISC-V toolchain pin mismatch at ${TC_DIR}."
    verify_toolchain
    ok "RISC-V toolchain ready: ${TC_DIR} (${RISCV_TOOLCHAIN_TRIPLE}, GCC ${RISCV_TOOLCHAIN_GCC_MAJOR})"
    exit 0
fi

if [[ ! -x "${GCC}" || "${INSTALLED_STAMP}" != "${EXPECTED_STAMP}" ]]; then
...
verify_toolchain
ok "RISC-V toolchain ready: ${TC_DIR} (${RISCV_TOOLCHAIN_TRIPLE}, GCC ${RISCV_TOOLCHAIN_GCC_MAJOR})"
info "Native rp2350-riscv build:  export PICO_TOOLCHAIN_PATH=${TC_DIR}"
