 ...
 *
 * Built-in command:
 * - `HELLO` -> `OK HELLO module=<name> proto=1 session=<id> fw=<ver> build=<id>
 * uid=<hex>`
 *
...
typedef struct {
  ...
  char line[HAL_SERIAL_SESSION_MAX_LINE +
            1u];          /**< Command line buffer (NUL-terminated on parse). */
  ...
  char uid_hex[HAL_DEVICE_UID_HEX_BUF_SIZE]; /**< Cached device UID hex string.
                                              */
  hal_serial_session_unknown_cb_t
      unknown_handler; /**< Optional sink for non-protocol command lines. */
  void *
      unknown_user; /**< Opaque user pointer forwarded to @c unknown_handler. */
  bool in_request;  /**< true while dispatching a framed request (gates
                       `hal_serial_session_println`). */
  uint16_t request_seq; /**< Sequence number of the in-flight framed request. */
  const hal_serial_session_vocabulary_t
      *vocab; /**< Optional override of inbound/outbound SC tokens (NULL ->
                 defaults). */
#ifdef HAL_ENABLE_CRYPTO
  ...
  uint8_t uid_bytes[HAL_DEVICE_UID_BYTES]; /**< Cached binary device UID (used
                                              for K_device). */
  bool authenticated; /**< true once a valid AUTH_PROVE has been seen for the
                         current session. */
  bool challenge_pending; /**< true between AUTH_BEGIN and the next AUTH_PROVE
                             (or session reset). */
  uint8_t challenge[HAL_SC_AUTH_CHALLENGE_BYTES]; /**< Last issued challenge
                                                     nonce. */
  uint32_t
      auth_counter; /**< Monotonic counter mixed into challenge derivation. */
  uint32_t auth_failures; /**< Counter of failed AUTH_PROVE attempts (for Phase
                             7 lockout). */
 ...
 *        and (optionally) install a custom @ref
...
static inline void hal_serial_session_init_with_vocabulary(
  ...
  session->module_tag =
      (module_tag != NULL) ? module_tag : HAL_SERIAL_SESSION_UNKNOWN;
 ...
 * NULL for @c vocab. Kept as the primary entry point for callers that do
 ...
 */
static inline void
hal_serial_session_set_unknown_handler(hal_serial_session_t *session,
 ...
 */
static inline bool
hal_serial_session_is_active(const hal_serial_session_t *session) {
...
hal_serial_session_id(const hal_serial_session_t *session) {
 ...
 */
static inline bool
hal_serial_session_is_authenticated(const hal_serial_session_t *session) {
#ifdef HAL_ENABLE_CRYPTO
 ...
 */
static inline bool
hal_serial_session_should_mute_debug_for_line(const char *line) {
  if (line == NULL) {
    return false;
  }
  return strncmp(line, HAL_SERIAL_FRAME_PREFIX, HAL_SERIAL_FRAME_PREFIX_LEN) ==
         0;
}
 ...
 */
static inline void
hal_serial_session__emit_hello(hal_serial_session_t *session) {
  char response[192];
 ...
 */
static inline void
hal_serial_session__reset_auth(hal_serial_session_t *session) {
...
/**
 * @brief Internal: parse exactly 2*N lowercase/uppercase hex characters into N
 * bytes.
 * @return true on success.
 */
static inline bool hal_serial_session__hex_decode(const char *hex, uint8_t *out,
                                                  size_t out_len) {
 ...
 */
static inline void
hal_serial_session__generate_challenge(hal_serial_session_t *session) {
 ...
 */
static inline void
hal_serial_session__handle_auth_begin(hal_serial_session_t *session) {
  if (!session->active) {
    hal_serial_session_println(
        session,
  ...
  const char *fmt = HAL_SERIAL_SESSION_VOCAB(session, reply_auth_challenge_fmt);
  if (fmt == NULL) {
    return;
  }
  char response[64];
 ...
 */
static inline void
hal_serial_session__handle_auth_prove(hal_serial_session_t *session,
                                      const char *args) {
  if (!session->active) {
    hal_serial_session_println(
        session,
  ...
  if (!hal_serial_session__hex_decode(args, provided, sizeof(provided))) {
    ...
    session->auth_failures++;
    hal_serial_session_println(
        session, HAL_SERIAL_SESSION_VOCAB(session, reply_auth_failed_bad_hex));
    return;
  }

  uint8_t key[HAL_SC_AUTH_KEY_BYTES];
  if (!hal_sc_auth_derive_device_key(session->uid_bytes, HAL_DEVICE_UID_BYTES,
                                     key)) {
    session->challenge_pending = false;
    session->auth_failures++;
    hal_serial_session_println(
  ...
  if (!hal_sc_auth_compute_response(key, session->challenge,
  ...
  if (!hal_sc_auth_macs_equal(provided, expected, sizeof(expected))) {
    session->auth_failures++;
    hal_serial_session_println(
        session, HAL_SERIAL_SESSION_VOCAB(session, reply_auth_failed_bad_mac));
    return;
  }
 ...
 */
static inline void
hal_serial_session__handle_reboot_bootloader(hal_serial_session_t *session) {
 ...
 */
static inline void
hal_serial_session__handle_bye(hal_serial_session_t *session) {
  const char *reply_bye_ok = HAL_SERIAL_SESSION_VOCAB(session, reply_bye_ok);
 ...
 */
static inline void
hal_serial_session__dispatch_inner(hal_serial_session_t *session,
                                   const char *inner) {
  if (strcmp(inner, "HELLO") == 0) {
    ...
    session->session_id = ((uint32_t)session->hello_counter << 20) ^
  ...
  if (cmd_auth_prove != NULL) {
    ...
    if (strncmp(inner, cmd_auth_prove, cmd_auth_prove_len) == 0 &&
      ...
      hal_serial_session__handle_auth_prove(session,
                                            inner + cmd_auth_prove_len);
      return;
    }
  }
...
static inline void hal_serial_session_poll(hal_serial_session_t *session) {
  ...
  while (hal_serial_available() > 0) {
    ...
    inner[0] = '\0';
    if (hal_serial_frame_decode(session->line, &seq, inner, sizeof(inner))) {
