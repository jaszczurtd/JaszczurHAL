...
static bool pcf8563_write_regs(const pcf8563_t *dev, uint8_t reg,
...
static bool pcf8563_read_regs(const pcf8563_t *dev, uint8_t reg, uint8_t *data,
                              uint8_t count) {
  if (!dev || !data || count == 0u) {
    return false;
  }

  return hal_i2c_write_read_bus(dev->i2c_bus, dev->i2c_addr, &reg, 1u, data,
                                count);
}

static bool pcf8563_encode_clkout_mode(pcf8563_clkout_mode_t mode,
                                       uint8_t *out_reg) {
  if (!out_reg) {
    return false;
  }
...
static bool pcf8563_decode_clkout_mode(uint8_t reg,
...
static bool pcf8563_encode_timer_clock(pcf8563_timer_clock_t timer_clock,
                                       uint8_t *out_reg) {
  if (!out_reg) {
    return false;
  }
...
bool pcf8563_get_datetime(const pcf8563_t *dev, pcf8563_datetime_t *out_dt) {
  ...
  out_dt->second =
      pcf8563_from_bcd((uint8_t)(buffer[0] & PCF8563_SECONDS_MASK));
  out_dt->minute =
      pcf8563_from_bcd((uint8_t)(buffer[1] & PCF8563_MINUTES_MASK));
...
bool pcf8563_set_datetime(const pcf8563_t *dev, const pcf8563_datetime_t *dt) {
  ...
  return pcf8563_write_regs(dev, PCF8563_REG_SECONDS, buffer,
                            (uint8_t)sizeof(buffer));
}
...
bool pcf8563_get_and_clear_flags(const pcf8563_t *dev, bool *out_alarm_flag,
  ...
  *out_timer_flag = (cs2 & PCF8563_CS2_TF) != 0u;
  ...
  const uint8_t cleared =
      (uint8_t)(cs2 & (PCF8563_CS2_TI_TP | PCF8563_CS2_AIE | PCF8563_CS2_TIE));
...
bool pcf8563_set_timer(const pcf8563_t *dev, pcf8563_timer_clock_t timer_clock,
  ...
  return pcf8563_write_regs(dev, PCF8563_REG_TIMER_CONTROL, buffer,
                            (uint8_t)sizeof(buffer));
}
...
bool pcf8563_set_alarm(const pcf8563_t *dev, const pcf8563_alarm_t *alarm) {
  ...
  return pcf8563_write_regs(dev, PCF8563_REG_ALARM_MINUTE, buffer,
                            (uint8_t)sizeof(buffer));
}
