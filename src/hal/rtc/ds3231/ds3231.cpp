...
bool DS3231::readBytes(byte reg, byte *data, uint8_t count) {
  ...
  return hal_i2c_write_read_bus(_i2c_bus, _i2c_addr, &reg, 1u, data, count);
}
...
void DS3231::setA1Time(byte A1Day, byte A1Hour, byte A1Minute, byte A1Second,
  ...
  byte temp_buffer = 0u;
  const byte data0[] = {
      0x07u, (byte)(decToBcd(A1Second) | ((AlarmBits & 0b00000001u) << 7))};
  (void)writeBytes(data0[0], &data0[1], 1u);

  const byte data1[] = {
      0x08u, (byte)(decToBcd(A1Minute) | ((AlarmBits & 0b00000010u) << 6))};
...
void DS3231::setA2Time(byte A2Day, byte A2Hour, byte A2Minute, byte AlarmBits,
                       bool A2Dy, bool A2h12, bool A2PM) {
  byte temp_buffer = 0u;
  const byte data0[] = {
      0x0Bu, (byte)(decToBcd(A2Minute) | ((AlarmBits & 0b00010000u) << 3))};
  (void)writeBytes(data0[0], &data0[1], 1u);
...
bool DS3231::checkIfAlarm(byte Alarm) { return checkIfAlarm(Alarm, true); }

bool DS3231::checkIfAlarm(byte Alarm, bool clearflag) {
  byte temp_buffer = readControlByte(1);
  const bool result = (Alarm == 1u) ? ((temp_buffer & 0b00000001u) != 0u)
