...
void MFRC522_BUS_DEVICE::PCD_WriteRegister(MFRC522::PCD_Register reg,
                                           byte value) {
  (void)reg;
  (void)value;
}
...
byte MFRC522_BUS_DEVICE::PCD_ReadRegister(MFRC522::PCD_Register reg) {
  (void)reg;
  return 0;
}

void MFRC522_BUS_DEVICE::PCD_ReadRegister(MFRC522::PCD_Register reg, byte count,
...
void MFRC522::PCD_WriteRegister(MFRC522::PCD_Register reg, byte count,
                                byte *values) {
...
void MFRC522::PCD_ReadRegister(MFRC522::PCD_Register reg, byte count,
                               byte *values, byte rxAlign) {
...
MFRC522::StatusCode MFRC522::PCD_CalculateCRC(
  ...
  _dev->PCD_WriteRegister(DivIrqReg,
                          0x04); // Clear the CRCIRq interrupt request bit
  _dev->PCD_WriteRegister(FIFOLevelReg,
...
void MFRC522::PCD_Init() {
  ...
  _dev->PCD_WriteRegister(
      TxASKReg, 0x40); // Default 0x00. Force a 100 % ASK modulation independent
...
void MFRC522::PCD_Reset() {
  _dev->PCD_WriteRegister(CommandReg,
...
bool MFRC522::PCD_PerformSelfTest() {
  ...
  _dev->PCD_WriteRegister(FIFODataReg, 25,
...
void MFRC522::PCD_SoftPowerDown() { // Note : Only soft power down mode is
  ...
  byte val =
      _dev->PCD_ReadRegister(CommandReg); // Read state of the command register
...
void MFRC522::PCD_SoftPowerUp() {
  ...
  const uint32_t timeout =
      (uint32_t)hal_millis() + 500; // create timer for timeout (just in case)

  while (hal_millis() <= timeout) { // set timeout to 500 ms
    val = _dev->PCD_ReadRegister(
        CommandReg);         // Read state of the command register
...
MFRC522::StatusCode MFRC522::PCD_CommunicateWithPICC(
  ...
  _dev->PCD_WriteRegister(CommandReg, PCD_Idle); // Stop any active command.
  _dev->PCD_WriteRegister(ComIrqReg,
                          0x7F); // Clear all seven interrupt request bits
  _dev->PCD_WriteRegister(FIFOLevelReg,
                          0x80); // FlushBuffer = 1, FIFO initialization
  _dev->PCD_WriteRegister(FIFODataReg, sendLen,
  ...
  byte errorRegValue = _dev->PCD_ReadRegister(
  ...
  if (backData && backLen) {
    byte n =
        _dev->PCD_ReadRegister(FIFOLevelReg); // Number of bytes in the FIFO
    if (n > *backLen) {
      return STATUS_NO_ROOM;
    }
    *backLen = n; // Number of bytes returned
    _dev->PCD_ReadRegister(FIFODataReg, n, backData,
                           rxAlign); // Get received data from FIFO
    _validBits =
        _dev->PCD_ReadRegister(ControlReg) &
...
MFRC522::StatusCode MFRC522::PICC_Select(
  ...
  // Level: PICC_CMD_SEL_CL1, PICC_CMD_SEL_CL2 or PICC_CMD_SEL_CL3
  // Byte 1: NVB Number of Valid Bits (in complete command, not just the UID):
  // High nibble: complete bytes, Low nibble: Extra bits. 		Byte 2:
  // UID-data or CT See explanation below. CT means Cascade Tag.
  // Byte 3: UID-data 		Byte 4: UID-data 		Byte 5: UID-data
  // Byte 6: BCC Block Check Character - XOR of bytes 2-5 		Byte 7:
  // CRC_A 		Byte 8: CRC_A
  ...
  while (!uidComplete) {
    ...
    while (!selectDone) {
      ...
      if (result ==
        ...
        byte valueOfCollReg = _dev->PCD_ReadRegister(
...
hal_status_t MFRC522::StatusCodeToHalStatus(MFRC522::StatusCode code) {
  switch (code) {
  case STATUS_OK:
    return HAL_OK;
  case STATUS_COLLISION:
...
byte MFRC522::PCD_GetVersion() { return _dev->PCD_ReadRegister(VersionReg); }
