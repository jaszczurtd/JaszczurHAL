...
static inline void timer_set_state_atomic(hal_timer_impl_t *t, hal_timer_state_t s) {
    __atomic_store_n((int *)&t->state, (int)s, __ATOMIC_RELEASE);
}

static inline hal_timer_state_t timer_get_state_atomic(const hal_timer_impl_t *t) {
    return (hal_timer_state_t)__atomic_load_n((const int *)&t->state, __ATOMIC_ACQUIRE);
}
...
hal_timer_result_t hal_timer_create(hal_timer_pool_t pool,
                                    uint32_t period_us,
                                    bool periodic,
                                    hal_timer_callback_t callback,
                                    void *user_data,
                                    hal_timer_t *out_timer) {
...
hal_timer_result_t hal_timer_start(hal_timer_t timer) {
    ...
    hal_alarm_id_t id = hal_timer_pool_add_alarm_us_ex(timer->pool,
                                                        period,
                                                        timer_internal_alarm_cb,
                                                        timer,
                                                        false,
                                                        &add_res);
...
hal_timer_result_t hal_timer_set_period_us(hal_timer_t timer,
    ...
    hal_timer_result_t add_res = HAL_TIMER_OK;
    hal_alarm_id_t new_id = hal_timer_pool_add_alarm_us_ex(timer->pool,
                                                            period_us,
                                                            timer_internal_alarm_cb,
                                                            timer,
                                                            false,
