...
static kv_bank_hdr_t bank_hdr_from_eeprom(uint16_t addr) {
    ...
    h.version = raw[2];
    h.reserved = raw[3];
    h.generation = (uint32_t)raw[4] | ((uint32_t)raw[5] << 8) | ((uint32_t)raw[6] << 16) | ((uint32_t)raw[7] << 24);
...
static bool value_equals_at(uint16_t payload_addr, const uint8_t *data, uint16_t len) {
    ...
    while (off < len) {
        ...
        if (memcmp(chunk, data + off, n) != 0) return false;
        off = (uint16_t)(off + n);
    }
    return true;
}
    ...
    if (len > 0) {
        if (data) {
            ...
            while (off < len) {
                ...
                for (uint16_t i = 0; i < n; i++) vcrc = crc16_ccitt_update(vcrc, 0);
                off = (uint16_t)(off + n);
            }
        }
    }
