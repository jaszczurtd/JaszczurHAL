 ...
 */
typedef bool (*hal_modem_at_ready_cb_t)(const char *buf, size_t len,
                                        void *user);
 ...
 *
 * @param h Handle obtained from hal_modem_at_create(). Must not be used after
 * this call.
 */
 ...
 *
 * @param h           Handle.
 * @param cmd         AT command without trailing CR/LF (e.g. "AT+CGMI"). Must
 * not be NULL.
 * @param expected    Optional substring that signals success early (NULL to use
 * OK/ERROR only).
 ...
 *
 * @param h                  Handle.
 * @param cmd                AT command without trailing CR/LF. Must not be
 * NULL.
 * @param data               Payload bytes (may contain any byte value,
 * including 0x00). Must not be NULL when data_len > 0.
 * @param data_len           Number of payload bytes.
 * @param prompt_timeout_ms  Max time to wait for '>' (typical: 1000 ms).
 * @param resp_timeout_ms    Max time to wait for OK/ERROR after payload
 * (typical: 5000 ms).
 * @return One of hal_modem_at_result_t.
 */
 ...
 * @param total_timeout_ms  Hard upper bound on the total wait (typical: 15000
 * ms).
 ...
 *
 * @param h       Handle.
 * @param prefix  Line prefix to match (e.g. "+CMQTTRXSTART:"). Must not be
 * NULL.
