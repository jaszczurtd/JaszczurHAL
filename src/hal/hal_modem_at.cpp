...
struct hal_modem_at_impl_s {
    ...
    hal_modem_at_urc_cb_t line_observer;  /* raw per-line tap, see set_line_observer */
...
static inline void engine_poll_sleep(hal_modem_at_impl_t *h) {
    if (h->tick_cb) h->tick_cb(h->tick_user);
    hal_delay_ms(2);
}
...
/* Decide whether the current rx_buf already constitutes a final answer.
   ...
   Semantics:
     ...
     -1  -> ERROR terminator seen (always, regardless of `expected`).
      0  -> no terminator yet                                            */
static int response_terminated(const hal_modem_at_impl_t *h, const char *expected) {
    if (expected) {
        if (buf_contains(h->cfg.rx_buf, h->rx_len, expected)) return 1;
    } else {
        /* OK / ERROR are case-sensitive in the SimCom AT protocol. */
        if (buf_contains(h->cfg.rx_buf, h->rx_len, "\r\nOK\r\n")) return 1;
    }
    if (buf_contains(h->cfg.rx_buf, h->rx_len, "\r\nERROR\r\n")) return -1;
    if (buf_contains(h->cfg.rx_buf, h->rx_len, "+CME ERROR"))    return -1;
    if (buf_contains(h->cfg.rx_buf, h->rx_len, "+CMS ERROR"))    return -1;
    return 0;
}

/* Render a TX line into a heap-free local buffer with any registered
   secrets replaced by "***", then log via hal_deb. */
static void log_filtered(hal_modem_at_impl_t *h, const char *tag, const char *line) {
    if (!hal_deb_is_initialized()) return;
    char scratch[160];
    size_t in_len = strlen(line);
    if (in_len >= sizeof(scratch)) in_len = sizeof(scratch) - 1;
    ...
    for (size_t i = 0; i < h->secret_count; i++) {
        ...
        while ((p = strstr(p, s)) != NULL) {
            ...
            if (off + (ptrdiff_t)slen > cur_total) break;
            if (delta != 0) {
                if (cur_total + delta + 1 > (ptrdiff_t)sizeof(scratch)) break;
...
hal_modem_at_t hal_modem_at_create(const hal_modem_at_config_t *cfg) {
    ...
    if (slot < 0) {
        HAL_ASSERT(0, "hal_modem_at: pool exhausted - increase HAL_MODEM_AT_MAX_INSTANCES");
        return NULL;
    }
    ...
    h->secret_count = 0;
    h->cfg = *cfg;
    if (h->cfg.default_timeout_ms == 0) h->cfg.default_timeout_ms = 1000u;
    if (h->cfg.quiet_window_ms    == 0) h->cfg.quiet_window_ms    = 250u;
...
hal_modem_at_result_t hal_modem_at_send(hal_modem_at_t h,
                                        ...
                                        const char *expected,
                                        uint32_t timeout_ms) {
    if (!h || !h->in_use || !cmd) return HAL_MODEM_AT_INVALID_ARG;
    if (timeout_ms == 0) timeout_ms = h->cfg.default_timeout_ms;
    ...
    for (;;) {
        ...
        if (term > 0) { res = HAL_MODEM_AT_OK; break; }
        if (term < 0) { res = HAL_MODEM_AT_ERROR; break; }
        if ((hal_millis() - start) >= timeout_ms) break;
        engine_poll_sleep(h);
    }
...
hal_modem_at_result_t hal_modem_at_send_with_data(hal_modem_at_t h,
    ...
    if (!h || !h->in_use || !cmd) return HAL_MODEM_AT_INVALID_ARG;
    if (data_len > 0 && !data)    return HAL_MODEM_AT_INVALID_ARG;
    if (prompt_timeout_ms == 0) prompt_timeout_ms = 1000u;
    if (resp_timeout_ms   == 0) resp_timeout_ms   = h->cfg.default_timeout_ms;
    ...
    for (;;) {
        ...
        if (term > 0) { res = HAL_MODEM_AT_OK; break; }
        if (term < 0) { res = HAL_MODEM_AT_ERROR; break; }
        if ((hal_millis() - start) >= resp_timeout_ms) break;
        engine_poll_sleep(h);
    }
...
hal_modem_at_result_t hal_modem_at_listen_until(hal_modem_at_t h,
    ...
    for (;;) {
        ...
        /* Stop condition: predicate fired AND the stream went quiet. */
        if (ready_seen && (now - last_byte_ms) >= h->cfg.quiet_window_ms) {
            res = HAL_MODEM_AT_OK;
            break;
        }
        /* Stop condition for passive settle (no predicate): quiet window
           is enough on its own. */
        if (!ready && consumed == 0 &&
            (now - start) >= h->cfg.quiet_window_ms) {
            res = HAL_MODEM_AT_OK;
            break;
        }
...
bool hal_modem_at_urc_register(hal_modem_at_t h,
                               ...
                               hal_modem_at_urc_cb_t cb,
                               void *user) {
    if (!h || !prefix || !*prefix) return false;
    size_t plen = strlen(prefix);
    if (plen >= HAL_MODEM_AT_URC_PREFIX_MAX) return false;
...
int hal_modem_at_urc_poll(hal_modem_at_t h) {
    ...
    int lines = 0;
    if (consumed > 0) {
        size_t start = h->rx_len > (size_t)consumed
                       ? h->rx_len - (size_t)consumed
                       : 0u;
...
void hal_modem_at_set_log_filter(hal_modem_at_t h,
    ...
    if (count == 0 || !secrets) {
        ...
        if (count > HAL_MODEM_AT_MAX_SECRETS) count = HAL_MODEM_AT_MAX_SECRETS;
        h->secrets = secrets;
