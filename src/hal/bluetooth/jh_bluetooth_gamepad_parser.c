...
enum {
  ...
  JH_HID_INPUT_REGION_CAPACITY = 24u,
};
...
typedef struct {
  ...
  uint16_t bit_length;
} jh_hid_input_region_t;

typedef struct {
  jh_hid_input_region_t regions[JH_HID_INPUT_REGION_CAPACITY];
  uint8_t region_count;
  bool report_ids_declared;
} jh_hid_descriptor_scan_t;
...
static uint32_t local_usage_value(const hid_descriptor_item_t *item,
                                  uint16_t usage_page) {
  const uint32_t value = (uint32_t)item->item_value;
  return item->data_size > 2u
             ? value
             : ((uint32_t)usage_page << 16u) | (value & UINT32_C(0xffff));
}
...
static bool accepted_application_usage(uint32_t usage) {
  ...
  return usage_page == HID_USAGE_PAGE_DESKTOP &&
         (usage_id == JH_HID_USAGE_GAME_PAD ||
          usage_id == JH_HID_USAGE_JOYSTICK);
}

static bool append_input_region(jh_hid_descriptor_scan_t *scan,
                                uint16_t report_id, uint16_t bit_position,
                                uint16_t bit_length) {
  if (scan->region_count >= JH_HID_INPUT_REGION_CAPACITY) {
    return false;
  }
  jh_hid_input_region_t *region = &scan->regions[scan->region_count++];
  region->report_id = report_id;
  region->bit_position = bit_position;
  region->bit_length = bit_length;
  return true;
}

static hal_status_t scan_descriptor(jh_bluetooth_gamepad_parser_t *parser,
                                    const uint8_t *descriptor,
                                    uint16_t descriptor_length,
                                    jh_hid_descriptor_scan_t *out_scan) {
  btstack_hid_descriptor_iterator_t iterator;
  btstack_hid_descriptor_iterator_init(&iterator, descriptor,
                                       descriptor_length);
  ...
  jh_hid_globals_t globals = {
      ...
      .logical_maximum = 0,
      .usage_page = 0u,
      .report_id = HID_REPORT_ID_UNDEFINED,
      .report_size = 0u,
      .report_count = 0u,
  };
  ...
  while (btstack_hid_descriptor_iterator_has_more(&iterator)) {
    const hid_descriptor_item_t *item =
        btstack_hid_descriptor_iterator_get_item(&iterator);
    if (item->item_type == Global) {
      switch ((GlobalItemTag)item->item_tag) {
      case UsagePage:
        if (item->item_value < 0 || item->item_value > UINT16_MAX) {
          return HAL_EPROTO;
        }
        globals.usage_page = (uint16_t)item->item_value;
        break;
      case LogicalMinimum:
        globals.logical_minimum = item->item_value;
        break;
      case LogicalMaximum:
        globals.logical_maximum = item->item_value;
        break;
      case ReportSize:
        if (item->item_value <= 0 || item->item_value > 32) {
          return HAL_EPROTO;
        }
        globals.report_size = (uint8_t)item->item_value;
        break;
      case ReportID:
        if (item->item_value <= 0 || item->item_value > UINT8_MAX) {
          return HAL_EPROTO;
        }
        globals.report_id = (uint16_t)item->item_value;
        out_scan->report_ids_declared = true;
        break;
      case ReportCount:
        if (item->item_value <= 0 || item->item_value > UINT8_MAX) {
          return HAL_EPROTO;
        }
        globals.report_count = (uint8_t)item->item_value;
        break;
      case Push:
    ...
    if (item->item_type == Local) {
      if (item->item_tag == Usage) {
        last_usage = local_usage_value(item, globals.usage_page);
        have_usage = true;
      }
      continue;
    }
    ...
    const MainItemTag main_tag = (MainItemTag)item->item_tag;
    if (main_tag == Coll) {
      ...
      const bool this_accepted =
          parent_accepted ||
          ((uint32_t)item->item_value == JH_HID_COLLECTION_APPLICATION &&
           have_usage && accepted_application_usage(last_usage));
      accepted_collections[collection_depth++] = this_accepted;
      found_gamepad = found_gamepad || this_accepted;
    } else if (main_tag == EndColl) {
      if (collection_depth == 0u) {
        return HAL_EPROTO;
      }
      --collection_depth;
    } else if (main_tag == Input) {
      if (globals.report_size == 0u || globals.report_count == 0u) {
        return HAL_EPROTO;
      }
      if (globals.report_id == HID_REPORT_ID_UNDEFINED) {
      ...
      const uint32_t total_bytes =
          ((total_bits + 7u) / 8u) +
          (globals.report_id == HID_REPORT_ID_UNDEFINED ? 0u : 1u);
      ...
      const bool inside_gamepad =
          collection_depth > 0u && accepted_collections[collection_depth - 1u];
      if (inside_gamepad &&
          (((uint32_t)item->item_value & JH_HID_MAIN_FLAG_CONSTANT) == 0u) &&
          !append_input_region(out_scan, globals.report_id, layout->bit_length,
                               (uint16_t)item_bits)) {
        return HAL_EOVERFLOW;
      }
  ...
  if (!btstack_hid_descriptor_iterator_valid(&iterator) ||
      collection_depth != 0u || global_depth != 0u) {
    return HAL_EPROTO;
  }
  if (!found_gamepad) {
    return HAL_EUNSUPPORTED;
  }
  if (parser->report_layout_count == 0u) {
    return HAL_EPROTO;
  }
  return HAL_OK;
}

static bool field_inside_gamepad(const jh_hid_descriptor_scan_t *scan,
                                 const btstack_hid_usage_item_t *item) {
  const uint32_t field_end = (uint32_t)item->bit_pos + item->size;
  for (uint8_t index = 0u; index < scan->region_count; ++index) {
    const jh_hid_input_region_t *region = &scan->regions[index];
    const uint32_t region_end =
        (uint32_t)region->bit_position + region->bit_length;
    if (region->report_id == item->report_id &&
        item->bit_pos >= region->bit_position && field_end <= region_end) {
      return true;
    }
  }
  return false;
...
static hal_status_t collect_fields(jh_bluetooth_gamepad_parser_t *parser,
                                   const uint8_t *descriptor,
                                   uint16_t descriptor_length,
                                   const jh_hid_descriptor_scan_t *scan) {
  btstack_hid_usage_iterator_t iterator;
  btstack_hid_usage_iterator_init(&iterator, descriptor, descriptor_length,
                                  HID_REPORT_TYPE_INPUT);
  while (btstack_hid_usage_iterator_has_more(&iterator)) {
    const int32_t logical_maximum = iterator.global_logical_maximum;
    btstack_hid_usage_item_t item;
    btstack_hid_usage_iterator_get_item(&iterator, &item);
    if (!field_inside_gamepad(scan, &item)) {
      continue;
    }
    if (((uint32_t)item.descriptor_item.item_value &
         JH_HID_MAIN_FLAG_VARIABLE) == 0u) {
    ...
    if (item.size == 0u || item.size > 32u ||
        item.global_logical_minimum > logical_maximum) {
      return HAL_EPROTO;
    }
    ...
    jh_bluetooth_gamepad_field_t *field =
        &parser->fields[parser->field_count++];
    field->logical_minimum = item.global_logical_minimum;
    field->logical_maximum = logical_maximum;
    field->report_id = item.report_id;
    field->bit_position = item.bit_pos;
    field->bit_size = item.size;
    field->index = index;
    field->main_flags = (uint8_t)item.descriptor_item.item_value;
    field->kind = kind;
  }
  return parser->field_count == 0u ? HAL_EUNSUPPORTED : HAL_OK;
}
...
jh_bluetooth_gamepad_parser_configure(jh_bluetooth_gamepad_parser_t *parser,
  ...
  if (scan_status != HAL_OK) {
    ...
    return reject_descriptor(parser, scan_status, reason);
  }
  parser->report_ids_declared = scan.report_ids_declared;

  const hal_status_t field_status =
      collect_fields(parser, descriptor, (uint16_t)descriptor_length, &scan);
  if (field_status != HAL_OK) {
    const jh_bluetooth_gamepad_reject_reason_t reason =
        field_status == HAL_EEXIST ? JH_BLUETOOTH_GAMEPAD_REJECT_DUPLICATE_USAGE
        : field_status == HAL_EOVERFLOW
            ? JH_BLUETOOTH_GAMEPAD_REJECT_TOO_MANY_FIELDS
        : field_status == HAL_EUNSUPPORTED
            ? JH_BLUETOOTH_GAMEPAD_REJECT_UNSUPPORTED_COLLECTION
            : JH_BLUETOOTH_GAMEPAD_REJECT_DESCRIPTOR_MALFORMED;
    return reject_descriptor(parser, field_status, reason);
  }
...
static bool apply_field(jh_bluetooth_gamepad_snapshot_t *snapshot,
  ...
  const uint16_t bit_position =
      (uint16_t)(field->bit_position +
                 (field->report_id == HID_REPORT_ID_UNDEFINED ? 0u : 8u));
  const uint32_t raw =
...
jh_bluetooth_gamepad_parser_parse_input(jh_bluetooth_gamepad_parser_t *parser,
  ...
  const uint16_t report_id =
      parser->report_ids_declared ? report[0] : HID_REPORT_ID_UNDEFINED;
  jh_bluetooth_gamepad_report_layout_t *layout =
      find_report_layout(parser, report_id);
  if (layout == NULL) {
