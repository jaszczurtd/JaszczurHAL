...
#if HCI_OUTGOING_PRE_BUFFER_SIZE < JH_BLUETOOTH_HCI_FRAME_HEADER_SIZE
...
#endif
#if HCI_INCOMING_PRE_BUFFER_SIZE < JH_BLUETOOTH_HCI_FRAME_HEADER_SIZE
#undef HCI_INCOMING_PRE_BUFFER_SIZE
#define HCI_INCOMING_PRE_BUFFER_SIZE JH_BLUETOOTH_HCI_FRAME_HEADER_SIZE
#endif
...
static jh_bluetooth_hci_transport_t s_transport_runtime;
...
static uint8_t *const s_receive_buffer =
    &s_incoming[JH_INCOMING_PRE_BUFFER_ALIGNED -
                JH_BLUETOOTH_HCI_FRAME_HEADER_SIZE];
...
static void data_source_process(btstack_data_source_t *data_source,
  ...
  (void)jh_bluetooth_hci_transport_service(&s_transport_runtime);
}

static void transport_init(const void *config) {
  (void)config;
  (void)jh_bluetooth_hci_transport_init(
      &s_transport_runtime, jh_bluetooth_controller_backend(), s_receive_buffer,
      JH_BLUETOOTH_HCI_FRAME_HEADER_SIZE + HCI_INCOMING_PACKET_BUFFER_SIZE,
      receive_packet, NULL);
}

static int transport_open(void) {
  const hal_status_t status =
      jh_bluetooth_hci_transport_open(&s_transport_runtime);
  if (status != HAL_OK) {
    return (int)status;
  }

  bd_addr_t address = {0u};
  if (jh_bluetooth_hci_transport_address(&s_transport_runtime, address) !=
      HAL_OK) {
    (void)jh_bluetooth_hci_transport_close(&s_transport_runtime);
    return (int)HAL_EIO;
  }
...
static int transport_close(void) {
  ...
  const hal_status_t status =
      jh_bluetooth_hci_transport_close(&s_transport_runtime);
...
static int transport_send_packet(uint8_t packet_type, uint8_t *packet,
  ...
  const hal_status_t status = jh_bluetooth_hci_transport_send(
...
void jh_btstack_cyw43_transport_snapshot(
  ...
  jh_bluetooth_hci_transport_snapshot(&s_transport_runtime, out_snapshot);
}

void jh_btstack_cyw43_transport_invalidate(void) {
  jh_bluetooth_hci_transport_invalidate(&s_transport_runtime, HAL_EHW);
}
