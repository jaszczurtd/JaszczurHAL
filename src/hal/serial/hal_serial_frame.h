 ...
 */
static inline bool hal_serial_frame_encode(uint16_t seq, const char *payload,
  ...
  int written = snprintf(out, out_size, "%s%u,%s", HAL_SERIAL_FRAME_PREFIX,
                         (unsigned)seq, payload);
  ...
  const uint8_t crc =
      hal_serial_frame_crc8((const uint8_t *)out + 1, (size_t)written - 1u);

  const int extra = snprintf(out + written, out_size - (size_t)written, "*%02X",
                             (unsigned)crc);
 ...
 */
static inline bool hal_serial_frame_decode(const char *line, uint16_t *seq_out,
                                           char *payload_out,
  ...
  if (strncmp(line, HAL_SERIAL_FRAME_PREFIX, HAL_SERIAL_FRAME_PREFIX_LEN) !=
      0) {
    return false;
  }
  ...
  const uint8_t actual_crc =
      hal_serial_frame_crc8((const uint8_t *)(line + 1), crc_len);
