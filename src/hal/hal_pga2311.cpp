...
#include "hal_sync.h"
#include "impl/shared/pga2311/pga2311_driver.h"
...
hal_pga2311_t hal_pga2311_init(const hal_pga2311_config_t *cfg) {
    ...
    if (h == NULL) {
        HAL_ASSERT(0, "hal_pga2311: pool exhausted - increase HAL_PGA2311_MAX_INSTANCES");
        return NULL;
    }
    ...
    if (h->cfg.start_muted) {
        if (h->cfg.mute_pin != HAL_PGA2311_MUTE_PIN_NONE) {
            ...
            const bool ok = hal_pga2311_driver_write_codes(&h->cfg,
...
bool hal_pga2311_gain_half_db_to_raw(int16_t half_db, uint8_t *out_code) {
    ...
    if (code < (int16_t)HAL_PGA2311_CODE_MIN || code > (int16_t)HAL_PGA2311_CODE_MAX) {
        return false;
    }
...
bool hal_pga2311_set_gain_db(hal_pga2311_t h, float left_db, float right_db) {
    ...
    if (right_db < HAL_PGA2311_GAIN_DB_MIN || right_db > HAL_PGA2311_GAIN_DB_MAX) {
        return false;
    }
    return hal_pga2311_set_gain_half_db(h,
                                        db_to_half_db(left_db),
...
bool hal_pga2311_set_mute(hal_pga2311_t h, bool mute) {
    ...
    if (h->cfg.mute_pin != HAL_PGA2311_MUTE_PIN_NONE) {
        ...
        ok = hal_pga2311_driver_write_codes(&h->cfg,
                                            HAL_PGA2311_CODE_MUTE,
                                            HAL_PGA2311_CODE_MUTE);
    } else {
        ok = hal_pga2311_driver_write_codes(&h->cfg,
