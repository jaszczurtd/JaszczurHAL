...
void hal_mock_ota_set_begin_result(bool result) { s_ota.begin_result = result; }
...
uint16_t hal_mock_ota_get_port(void) { return s_ota.port; }

const char *hal_mock_ota_get_hostname(void) { return s_ota.hostname; }

const char *hal_mock_ota_get_password(void) { return s_ota.password; }

uint32_t hal_mock_ota_get_handle_count(void) { return s_ota.handle_count; }
...
bool hal_ota_is_started(void) { return s_ota.started; }

hal_status_t hal_ota_confirm_boot_ex(void) { return HAL_OK; }
...
hal_status_t hal_ota_get_boot_info_ex(hal_ota_boot_info_t *out_info) {
  ...
  out_info->mode = HAL_OTA_BOOT_STABLE;
  out_info->max_attempts = 3u;
  memcpy(out_info->program_version, "mock", 5u);
  memcpy(out_info->staging_version, "empty", 6u);
  return HAL_OK;
}
