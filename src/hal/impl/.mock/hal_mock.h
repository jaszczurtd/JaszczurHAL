 ...
 * RP2040 (hal_gpio_set_mode(OUTPUT) clobbers the output latch to 0, like
 * gpio_init()). To guard a driver against it, assert the *driven level* after a
 * drive-high, e.g. drive a pin HIGH then
 * TEST_ASSERT_TRUE(hal_mock_gpio_get_state(pin)) - with the buggy
 * write-then-mode order the level reads LOW. */
...
/** @brief Inject a chip temperature value returned by hal_read_chip_temp().
 * Default: 25.0 °C. */
void hal_mock_set_chip_temp(float celsius);
/** @brief Return true if hal_enter_bootloader() has been called in the mock
 * backend. */
...
typedef void (*hal_mock_uart_write_cb_t)(hal_uart_t h, const char *text,
                                         void *user);
/** @brief Install a callback fired after every write/println; pass NULL to
 * clear. */
...
/** @brief Control result returned by hal_wireguard_kick_handshake*() (default:
 * true). */
...
/** @brief Set the mock location-valid flag (returned by
 * hal_gps_location_is_valid()). */
void hal_mock_gps_set_valid(bool valid);
/** @brief Set the mock location-updated flag (returned by
 * hal_gps_location_is_updated()). */
void hal_mock_gps_set_updated(bool updated);
/** @brief Set the mock location age in milliseconds (returned by
...
/** @brief Return true if hal_eeprom_commit() has been called since the last
 * reset. */
...
/** @brief Return number of byte writes performed since last reset/counter
 * clear. */
...
/** @brief Return lock depth observed at the byte-read point for default I2C
 * bus. */
