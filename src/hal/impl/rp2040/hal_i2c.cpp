...
static void i2c_lock_idx(uint8_t idx) {
  const bool acquired = jh_i2c_recursive_lock_acquire(
      &s_i2c[idx].lock, i2c_current_owner_token());
  if (!acquired) {
...
static hal_status_t i2c_init_bus_common(uint8_t bus, uint8_t sda_pin,
  ...
  if (!jh_i2c_recursive_lock_init(&s_i2c[idx].lock)) {
    return HAL_ENOMEM;
  }
...
hal_status_t hal_i2c_get_clock_bus(uint8_t bus, uint32_t *out_clock_hz) {
  ...
  return jh_i2c_recursive_lock_read_u32(&st->lock, i2c_current_owner_token(),
                                        &st->clock_hz, out_clock_hz);
}
