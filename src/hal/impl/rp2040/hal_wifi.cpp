#include "../../hal_target.h"
#if HAL_TARGET_IS_RP2040
#include "../../hal_config.h"
#ifdef HAL_ENABLE_WIFI
#if !defined(HAL_NETWORK_BACKEND_CYW43)

#include "../../hal_net.h"
#include "../../hal_serial.h"
#include "../../hal_sync.h"
#include "../../hal_wifi.h"
#include "../shared/hal_mutex_once.h"
#include <WiFi.h>
#include <stdarg.h>
#include <stdio.h>
#include <string.h>

static hal_mutex_t s_wifi_mutex = NULL;
static uint32_t s_wifi_timeout_ms = 15000u;
static int s_wifi_scan_count = 0;
#if defined(HAL_NETWORK_BACKEND_CYW43)
static bool s_wifi_scan_running = false;
static bool s_wifi_join_running = false;
static bool s_wifi_join_pending = false;
static bool s_wifi_ping_running = false;
#endif

static inline void wifi_ensure_mutex(void) {
  (void)jh_hal_mutex_create_once(&s_wifi_mutex);
}

static bool validate_out(char *out, size_t out_size, const char *fn) {
  if (!out) {
    hal_derr("%s: output buffer is NULL", fn);
    return false;
  }
  if (out_size == 0) {
    hal_derr("%s: output buffer size is 0", fn);
    return false;
  }
  return true;
}

static bool checked_snprintf(char *out, size_t out_size, const char *fn,
                             const char *format, ...) {
  va_list args;
  va_start(args, format);
  int written = vsnprintf(out, out_size, format, args);
  va_end(args);

  if (written < 0) {
    hal_derr("%s: snprintf failed", fn);
    return false;
  }
  if ((size_t)written >= out_size) {
    hal_derr("%s: output buffer too small", fn);
    return false;
  }
  return true;
}

#if !defined(HAL_NETWORK_BACKEND_CYW43)
static const char *wifi_ssid_c_str(const char *ssid) {
  return ssid ? ssid : "";
}

template <typename T> static const char *wifi_ssid_c_str(const T &ssid) {
  return ssid.c_str();
}
#endif

#if !defined(HAL_NETWORK_BACKEND_CYW43)
static bool format_ipv4(char *out, size_t out_size, const char *fn,
                        const IPAddress &ip) {
  return checked_snprintf(out, out_size, fn, "%u.%u.%u.%u", (unsigned)ip[0],
                          (unsigned)ip[1], (unsigned)ip[2], (unsigned)ip[3]);
}
#endif

#if defined(HAL_NETWORK_BACKEND_CYW43)
static bool format_ipv4(char *out, size_t out_size, const char *fn,
                        const uint8_t ip[4]) {
  return checked_snprintf(out, out_size, fn, "%u.%u.%u.%u", (unsigned)ip[0],
                          (unsigned)ip[1], (unsigned)ip[2], (unsigned)ip[3]);
}
#endif

static bool format_mac(char *out, size_t out_size, const char *fn,
                       const uint8_t mac[6]) {
  return checked_snprintf(out, out_size, fn, "%02X:%02X:%02X:%02X:%02X:%02X",
                          (unsigned)mac[0], (unsigned)mac[1], (unsigned)mac[2],
                          (unsigned)mac[3], (unsigned)mac[4], (unsigned)mac[5]);
}

#if !defined(HAL_NETWORK_BACKEND_CYW43)
static hal_wifi_encryption_t map_encryption(uint8_t enc) {
  switch (enc) {
  case ENC_TYPE_NONE:
    return HAL_WIFI_ENC_NONE;
  case ENC_TYPE_TKIP:
    return HAL_WIFI_ENC_WPA;
  case ENC_TYPE_CCMP:
    return HAL_WIFI_ENC_WPA2;
  case ENC_TYPE_AUTO:
    return HAL_WIFI_ENC_AUTO;
  default:
    return HAL_WIFI_ENC_UNKNOWN;
  }
}
#endif

hal_status_t hal_wifi_set_mode_ex(hal_wifi_mode_t mode) {
#if defined(HAL_NETWORK_BACKEND_CYW43)
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  if (s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running) {
    hal_mutex_unlock(s_wifi_mutex);
    return HAL_EBUSY;
  }
  hal_status_t status;
  if (mode == HAL_WIFI_MODE_STA) {
    status = jh_rp2040_cyw43_provider_init();
  } else if (mode == HAL_WIFI_MODE_OFF) {
    status = jh_rp2040_cyw43_provider_leave();
    if (status == HAL_OK) {
      s_wifi_join_pending = false;
    }
  } else {
    status = mode == HAL_WIFI_MODE_AP || mode == HAL_WIFI_MODE_AP_STA
                 ? HAL_EUNSUPPORTED
                 : HAL_EINVAL;
  }
  hal_mutex_unlock(s_wifi_mutex);
  return status;
#else
  WiFiMode_t platform_mode;
  switch (mode) {
  case HAL_WIFI_MODE_OFF:
    platform_mode = WIFI_OFF;
    break;
  case HAL_WIFI_MODE_STA:
    platform_mode = WIFI_STA;
    break;
  case HAL_WIFI_MODE_AP:
    platform_mode = WIFI_AP;
    break;
  case HAL_WIFI_MODE_AP_STA:
    platform_mode = WIFI_AP_STA;
    break;
  default:
    hal_derr("hal_wifi_set_mode: invalid mode value %d", (int)mode);
    return HAL_EINVAL;
  }

  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  WiFi.mode(platform_mode);
  hal_mutex_unlock(s_wifi_mutex);
  return HAL_OK;
#endif
}

bool hal_wifi_set_mode(hal_wifi_mode_t mode) {
  return hal_status_to_bool(hal_wifi_set_mode_ex(mode));
}

hal_status_t hal_wifi_disconnect_ex(bool erase_credentials) {
#if defined(HAL_NETWORK_BACKEND_CYW43)
  (void)erase_credentials;
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  if (s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running) {
    hal_mutex_unlock(s_wifi_mutex);
    return HAL_EBUSY;
  }
  hal_status_t status = HAL_OK;
#if defined(HAL_ENABLE_TCP)
  status = jh_rp2040_tcp_reset_all();
#endif
#if defined(HAL_ENABLE_UDP)
  if (status == HAL_OK) {
    status = jh_rp2040_udp_reset_all();
  }
#endif
  if (status == HAL_OK) {
    status = jh_rp2040_cyw43_provider_leave();
  }
  if (status == HAL_OK) {
    s_wifi_join_pending = false;
  }
  hal_mutex_unlock(s_wifi_mutex);
  return status;
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  WiFi.disconnect(erase_credentials);
  hal_mutex_unlock(s_wifi_mutex);
  return HAL_OK;
#endif
}

bool hal_wifi_disconnect(bool erase_credentials) {
  return hal_status_to_bool(hal_wifi_disconnect_ex(erase_credentials));
}

hal_status_t hal_wifi_set_hostname_ex(const char *hostname) {
  if (!hostname || hostname[0] == '\0') {
    hal_derr("hal_wifi_set_hostname: hostname is NULL/empty");
    return HAL_EINVAL;
  }

  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
#if defined(HAL_NETWORK_BACKEND_CYW43)
  const hal_status_t status = jh_rp2040_cyw43_provider_set_hostname(hostname);
#else
  WiFi.setHostname(hostname);
#endif
  hal_mutex_unlock(s_wifi_mutex);
#if defined(HAL_NETWORK_BACKEND_CYW43)
  return status;
#else
  return HAL_OK;
#endif
}

bool hal_wifi_set_hostname(const char *hostname) {
  return hal_status_to_bool(hal_wifi_set_hostname_ex(hostname));
}

hal_status_t hal_wifi_begin_station_ex(const char *ssid, const char *password,
                                       bool non_blocking) {
  if (!ssid || ssid[0] == '\0') {
    hal_derr("hal_wifi_begin_station: SSID is NULL/empty");
    return HAL_EINVAL;
  }
  if (!password) {
    hal_derr("hal_wifi_begin_station: password pointer is NULL");
    return HAL_EINVAL;
  }

  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
#if defined(HAL_NETWORK_BACKEND_CYW43)
  if (s_wifi_scan_running || s_wifi_join_running || s_wifi_join_pending ||
      s_wifi_ping_running) {
    hal_mutex_unlock(s_wifi_mutex);
    return HAL_EBUSY;
  }
  s_wifi_join_running = true;
  const uint32_t timeout_ms = s_wifi_timeout_ms;
  hal_mutex_unlock(s_wifi_mutex);
  hal_status_t status = HAL_OK;
#if defined(HAL_ENABLE_TCP)
  status = jh_rp2040_tcp_reset_all();
#endif
#if defined(HAL_ENABLE_UDP)
  if (status == HAL_OK) {
    status = jh_rp2040_udp_reset_all();
  }
#endif
  if (status == HAL_OK) {
    status =
        jh_rp2040_cyw43_provider_join(ssid, password, non_blocking, timeout_ms);
  }
  hal_mutex_lock(s_wifi_mutex);
  s_wifi_join_running = false;
  s_wifi_join_pending = non_blocking && status == HAL_OK;
#else
  if (non_blocking) {
    WiFi.beginNoBlock(ssid, password);
  } else {
    WiFi.begin(ssid, password);
  }
#endif
  hal_mutex_unlock(s_wifi_mutex);
#if defined(HAL_NETWORK_BACKEND_CYW43)
  return status;
#else
  return HAL_OK;
#endif
}

bool hal_wifi_begin_station(const char *ssid, const char *password,
                            bool non_blocking) {
  return hal_status_to_bool(
      hal_wifi_begin_station_ex(ssid, password, non_blocking));
}

hal_status_t hal_wifi_set_timeout_ms_ex(uint32_t timeout_ms) {
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  s_wifi_timeout_ms = timeout_ms;
#if defined(HAL_NETWORK_BACKEND_CYW43)
  jh_rp2040_cyw43_provider_set_timeout_ms(timeout_ms);
#else
  WiFi.setTimeout(timeout_ms);
#endif
  hal_mutex_unlock(s_wifi_mutex);
  return HAL_OK;
}

bool hal_wifi_set_timeout_ms(uint32_t timeout_ms) {
  return hal_status_to_bool(hal_wifi_set_timeout_ms_ex(timeout_ms));
}

hal_status_t hal_wifi_get_state_ex(hal_wifi_state_t *out_state) {
  if (out_state == nullptr) {
    return HAL_EINVAL;
  }
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const int status = (int)WiFi.status();
  hal_mutex_unlock(s_wifi_mutex);
  switch (status) {
  case WL_CONNECTED:
    *out_state = HAL_WIFI_STATE_CONNECTED;
    break;
  case WL_NO_SSID_AVAIL:
    *out_state = HAL_WIFI_STATE_NO_NETWORK;
    break;
  case WL_CONNECT_FAILED:
    *out_state = HAL_WIFI_STATE_FAILED;
    break;
  case WL_DISCONNECTED:
    *out_state = HAL_WIFI_STATE_CONNECTED_NO_IP;
    break;
  default:
    *out_state = HAL_WIFI_STATE_IDLE;
    break;
  }
  return HAL_OK;
}

bool hal_wifi_is_connected(void) {
#if defined(HAL_NETWORK_BACKEND_CYW43)
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  if (s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running) {
    hal_mutex_unlock(s_wifi_mutex);
    return false;
  }
  jh_cyw43_link_status_t link_status = JH_CYW43_LINK_DOWN;
  const hal_status_t status =
      jh_rp2040_cyw43_provider_link_status(&link_status);
  if (status == HAL_OK && link_status != JH_CYW43_LINK_CONNECTING) {
    s_wifi_join_pending = false;
  }
  hal_mutex_unlock(s_wifi_mutex);
  return status == HAL_OK && link_status == JH_CYW43_LINK_JOINED;
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const bool connected = (WiFi.status() == WL_CONNECTED);
  hal_mutex_unlock(s_wifi_mutex);
  return connected;
#endif
}

int hal_wifi_status(void) {
#if defined(HAL_NETWORK_BACKEND_CYW43)
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  if (s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running) {
    hal_mutex_unlock(s_wifi_mutex);
    return WL_IDLE_STATUS;
  }
  jh_cyw43_link_status_t link_status = JH_CYW43_LINK_DOWN;
  if (jh_rp2040_cyw43_provider_link_status(&link_status) != HAL_OK) {
    hal_mutex_unlock(s_wifi_mutex);
    return WL_NO_MODULE;
  }
  if (link_status != JH_CYW43_LINK_CONNECTING) {
    s_wifi_join_pending = false;
  }
  hal_mutex_unlock(s_wifi_mutex);
  switch (link_status) {
  case JH_CYW43_LINK_DOWN:
  case JH_CYW43_LINK_CONNECTING:
    return WL_IDLE_STATUS;
  case JH_CYW43_LINK_NO_IP:
    return WL_DISCONNECTED;
  case JH_CYW43_LINK_JOINED:
    return WL_CONNECTED;
  case JH_CYW43_LINK_FAILED:
  case JH_CYW43_LINK_NO_NETWORK:
  case JH_CYW43_LINK_BAD_AUTH:
    return WL_CONNECT_FAILED;
  default:
    return WL_NO_MODULE;
  }
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const int st = (int)WiFi.status();
  hal_mutex_unlock(s_wifi_mutex);
  return st;
#endif
}

bool hal_wifi_has_local_ip(void) {
#if defined(HAL_NETWORK_BACKEND_CYW43)
  uint8_t address[4] = {};
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const hal_status_t status =
      s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running
          ? HAL_EBUSY
          : jh_rp2040_cyw43_provider_get_local_ipv4(address);
  hal_mutex_unlock(s_wifi_mutex);
  return status == HAL_OK &&
         (address[0] | address[1] | address[2] | address[3]) != 0u;
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const bool has_ip = (WiFi.localIP() != IPAddress(0, 0, 0, 0));
  hal_mutex_unlock(s_wifi_mutex);
  return has_ip;
#endif
}

int32_t hal_wifi_rssi(void) {
#if defined(HAL_NETWORK_BACKEND_CYW43)
  int32_t rssi = 0;
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const hal_status_t status =
      s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running
          ? HAL_EBUSY
          : jh_rp2040_cyw43_provider_get_rssi(&rssi);
  hal_mutex_unlock(s_wifi_mutex);
  return status == HAL_OK ? rssi : 0;
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const int32_t rssi = WiFi.RSSI();
  hal_mutex_unlock(s_wifi_mutex);
  return rssi;
#endif
}

int hal_wifi_get_strength(void) {
  const int32_t rssi = hal_wifi_rssi();

  if (rssi >= 0)
    return 0;
  if (rssi >= -50)
    return 5;
  if (rssi >= -60)
    return 4;
  if (rssi >= -70)
    return 3;
  if (rssi >= -80)
    return 2;
  if (rssi >= -90)
    return 1;
  return 0;
}

hal_status_t hal_wifi_get_local_ip_ex(char *out, size_t out_size) {
  if (!validate_out(out, out_size, "hal_wifi_get_local_ip"))
    return HAL_EINVAL;

#if defined(HAL_NETWORK_BACKEND_CYW43)
  uint8_t ip[4] = {};
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const hal_status_t status =
      s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running
          ? HAL_EBUSY
          : jh_rp2040_cyw43_provider_get_local_ipv4(ip);
  hal_mutex_unlock(s_wifi_mutex);
  if (status != HAL_OK) {
    return status;
  }
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  IPAddress ip = WiFi.localIP();
  hal_mutex_unlock(s_wifi_mutex);
#endif

  return format_ipv4(out, out_size, "hal_wifi_get_local_ip", ip)
             ? HAL_OK
             : HAL_EOVERFLOW;
}

bool hal_wifi_get_local_ip(char *out, size_t out_size) {
  return hal_status_to_bool(hal_wifi_get_local_ip_ex(out, out_size));
}

hal_status_t hal_wifi_get_dns_ip_ex(char *out, size_t out_size) {
  if (!validate_out(out, out_size, "hal_wifi_get_dns_ip"))
    return HAL_EINVAL;

#if defined(HAL_NETWORK_BACKEND_CYW43)
  uint8_t ip[4] = {};
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const hal_status_t status =
      s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running
          ? HAL_EBUSY
          : jh_rp2040_cyw43_provider_get_dns_ipv4(ip);
  hal_mutex_unlock(s_wifi_mutex);
  if (status != HAL_OK) {
    return status;
  }
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  IPAddress ip = WiFi.dnsIP();
  hal_mutex_unlock(s_wifi_mutex);
#endif

  return format_ipv4(out, out_size, "hal_wifi_get_dns_ip", ip) ? HAL_OK
                                                               : HAL_EOVERFLOW;
}

bool hal_wifi_get_dns_ip(char *out, size_t out_size) {
  return hal_status_to_bool(hal_wifi_get_dns_ip_ex(out, out_size));
}

hal_status_t hal_wifi_get_mac_ex(char *out, size_t out_size) {
  if (!validate_out(out, out_size, "hal_wifi_get_mac"))
    return HAL_EINVAL;

  uint8_t mac[6] = {};
#if defined(HAL_NETWORK_BACKEND_CYW43)
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const hal_status_t status =
      s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running
          ? HAL_EBUSY
          : jh_rp2040_cyw43_provider_get_mac(mac);
  hal_mutex_unlock(s_wifi_mutex);
  if (status != HAL_OK) {
    return status;
  }
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  WiFi.macAddress(mac);
  hal_mutex_unlock(s_wifi_mutex);
#endif

  return format_mac(out, out_size, "hal_wifi_get_mac", mac) ? HAL_OK
                                                            : HAL_EOVERFLOW;
}

bool hal_wifi_get_mac(char *out, size_t out_size) {
  return hal_status_to_bool(hal_wifi_get_mac_ex(out, out_size));
}

int hal_wifi_ping(const char *host_or_ip) {
  int result = -1;
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const uint32_t timeout_ms = s_wifi_timeout_ms;
  hal_mutex_unlock(s_wifi_mutex);
  (void)hal_wifi_ping_status_ex(host_or_ip, timeout_ms, &result);
  return result;
}

hal_status_t hal_wifi_ping_status_ex(const char *host_or_ip,
                                     uint32_t timeout_ms, int *out_result) {
  if (out_result != NULL) {
    *out_result = -1;
  }
  if (!host_or_ip || host_or_ip[0] == '\0') {
    hal_derr("hal_wifi_ping_ex: host_or_ip is NULL/empty");
    return HAL_EINVAL;
  }
  if (out_result == NULL) {
    return HAL_EINVAL;
  }

#if defined(HAL_NETWORK_BACKEND_CYW43)
  if (timeout_ms == 0u) {
    return HAL_EINVAL;
  }
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  if (s_wifi_scan_running || s_wifi_join_running || s_wifi_ping_running) {
    hal_mutex_unlock(s_wifi_mutex);
    return HAL_EBUSY;
  }
  s_wifi_ping_running = true;
  hal_mutex_unlock(s_wifi_mutex);

  uint8_t address[HAL_NET_IPV4_ADDR_LEN] = {};
  hal_status_t status = hal_net_resolve_ipv4_ex(host_or_ip, address);
  if (status == HAL_OK) {
    status =
        jh_rp2040_cyw43_provider_ping_ipv4(address, timeout_ms, out_result);
  }

  hal_mutex_lock(s_wifi_mutex);
  s_wifi_ping_running = false;
  hal_mutex_unlock(s_wifi_mutex);
  return status;
#else
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
  const uint32_t previous_timeout_ms = s_wifi_timeout_ms;
  if (timeout_ms != previous_timeout_ms) {
    WiFi.setTimeout(timeout_ms);
  }
  *out_result = WiFi.ping(host_or_ip);
  if (timeout_ms != previous_timeout_ms) {
    WiFi.setTimeout(previous_timeout_ms);
  }
  hal_mutex_unlock(s_wifi_mutex);
  return *out_result >= 0 ? HAL_OK : HAL_EIO;
#endif
}

int hal_wifi_ping_ex(const char *host_or_ip, uint32_t timeout_ms) {
  int result = -1;
  (void)hal_wifi_ping_status_ex(host_or_ip, timeout_ms, &result);
  return result;
}

hal_status_t hal_wifi_scan_networks_ex(int *out_count) {
  if (out_count == NULL) {
    return HAL_EINVAL;
  }
  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
#if defined(HAL_NETWORK_BACKEND_CYW43)
  if (s_wifi_scan_running || s_wifi_join_running || s_wifi_join_pending ||
      s_wifi_ping_running) {
    hal_mutex_unlock(s_wifi_mutex);
    return HAL_EBUSY;
  }
  s_wifi_scan_running = true;
  const uint32_t timeout_ms = s_wifi_timeout_ms;
  hal_mutex_unlock(s_wifi_mutex);

  int scan_count = 0;
  const hal_status_t status =
      jh_rp2040_cyw43_provider_scan(timeout_ms, &scan_count);

  hal_mutex_lock(s_wifi_mutex);
  s_wifi_scan_count = scan_count;
  s_wifi_scan_running = false;
  *out_count = s_wifi_scan_count;
#else
  s_wifi_scan_count = WiFi.scanNetworks();
  *out_count = s_wifi_scan_count;
#endif
  hal_mutex_unlock(s_wifi_mutex);
#if defined(HAL_NETWORK_BACKEND_CYW43)
  return status;
#else
  return s_wifi_scan_count >= 0 ? HAL_OK : HAL_EIO;
#endif
}

int hal_wifi_scan_networks(void) {
  int count = -1;
  (void)hal_wifi_scan_networks_ex(&count);
  return count;
}

hal_status_t hal_wifi_get_scan_result_ex(size_t index,
                                         hal_wifi_scan_result_t *out) {
  if (out == NULL) {
    hal_derr("hal_wifi_get_scan_result: output pointer is NULL");
    return HAL_EINVAL;
  }

  wifi_ensure_mutex();
  hal_mutex_lock(s_wifi_mutex);
#if defined(HAL_NETWORK_BACKEND_CYW43)
  if (s_wifi_scan_running) {
    hal_mutex_unlock(s_wifi_mutex);
    return HAL_EBUSY;
  }
  const hal_status_t status =
      jh_rp2040_cyw43_provider_get_scan_result(index, out);
  hal_mutex_unlock(s_wifi_mutex);
  if (status == HAL_ENOENT) {
    hal_derr("hal_wifi_get_scan_result: index %u out of range",
             (unsigned)index);
  }
  return status;
#else
  if (s_wifi_scan_count <= 0 || index >= (size_t)s_wifi_scan_count) {
    hal_mutex_unlock(s_wifi_mutex);
    hal_derr("hal_wifi_get_scan_result: index %u out of range",
             (unsigned)index);
    return HAL_ENOENT;
  }

  bool ssid_ok =
      checked_snprintf(out->ssid, sizeof(out->ssid), "hal_wifi_get_scan_result",
                       "%s", wifi_ssid_c_str(WiFi.SSID((int)index)));
  WiFi.BSSID((int)index, out->bssid);
  out->encryption = map_encryption((uint8_t)WiFi.encryptionType((int)index));
  out->channel = (int32_t)WiFi.channel((int)index);
  out->rssi = (int32_t)WiFi.RSSI((int)index);
  hal_mutex_unlock(s_wifi_mutex);
  return ssid_ok ? HAL_OK : HAL_EOVERFLOW;
#endif
}

bool hal_wifi_get_scan_result(size_t index, hal_wifi_scan_result_t *out) {
  return hal_status_to_bool(hal_wifi_get_scan_result_ex(index, out));
}

const char *hal_wifi_encryption_to_string(hal_wifi_encryption_t encryption) {
  switch (encryption) {
  case HAL_WIFI_ENC_NONE:
    return "NONE";
  case HAL_WIFI_ENC_WPA:
    return "WPA";
  case HAL_WIFI_ENC_WPA2:
    return "WPA2";
  case HAL_WIFI_ENC_AUTO:
    return "AUTO";
  case HAL_WIFI_ENC_UNKNOWN:
  default:
    return "UNKN";
  }
}

#endif /* HAL_ENABLE_WIFI */
#endif /* !HAL_NETWORK_BACKEND_CYW43 */
#endif // HAL_TARGET_IS_RP2040
