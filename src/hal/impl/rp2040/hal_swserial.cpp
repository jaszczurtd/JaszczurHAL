...
struct hal_swserial_impl_s {
  ...
  bool started;
  int in_use;
  hal_swserial_impl_s(uint8_t rx, uint8_t tx)
      : sw(rx, tx), rx_pin(rx), tx_pin(tx), started(false), in_use(1) {}
};
...
static bool hal_swserial_rebind(hal_swserial_t h, uint8_t rx_pin,
                                uint8_t tx_pin) {
  if (!h || h->started)
    return false;
