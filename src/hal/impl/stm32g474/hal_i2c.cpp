...
void hal_i2c_init(uint8_t sda_pin, uint8_t scl_pin, uint32_t clock_hz) {
  const hal_status_t status = hal_i2c_init_ex(sda_pin, scl_pin, clock_hz);
  HAL_ASSERT(status == HAL_OK, "hal_i2c_init: failed to initialize bus");
}
...
void hal_i2c_init_bus(uint8_t bus, uint8_t sda_pin, uint8_t scl_pin,
  ...
  const hal_status_t status =
      hal_i2c_init_bus_ex(bus, sda_pin, scl_pin, clock_hz);
  HAL_ASSERT(status == HAL_OK,
             "hal_i2c_init_bus: invalid SDA/SCL pin mapping for selected bus");
}
