...
static hal_error_slot_t *get_error_slot(const char *source) {
    ...
    for (int i = 0; i < HAL_DEBUG_RATE_LIMIT_SOURCES_MAX; i++) {
        if (s_error_slots[i].in_use) {
            if (s_error_slots[i].hash == h &&
                strncmp(s_error_slots[i].source,
                        safe_source,
    ...
    if (free_idx < 0) {
        if (!s_overflow_slot.in_use) {
            ...
            copy_source_label(s_overflow_slot.source,
                              sizeof(s_overflow_slot.source),
                              "overflow");
...
void hal_debug_init(uint32_t baud, const hal_debug_rate_limit_t *cfg) {
    ...
    // practice, but guarding it keeps a double-init from orphaning the previous
    // mutexes and mirrors hal_serial_ensure_tx_mutex(). These statics are
    // zero-initialised (.bss), so testing against NULL is sound.
...
void hal_deb_set_prefix(const char *prefix) {
    if (prefix != NULL && strlen(prefix) > 0 &&
        ...
        strncpy(s_prefix, prefix, HAL_DEBUG_PREFIX_SIZE - 1u);
        s_prefix[HAL_DEBUG_PREFIX_SIZE - 1u] = '\0';
    }
}
...
void hal_debug_loop(void) {
     ...
     * the TX mutex and writes via stdio, so it must run from task
