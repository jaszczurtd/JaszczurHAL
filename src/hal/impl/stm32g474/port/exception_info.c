...
static jh_exception_info_t s_exc;
 ...
 */
__attribute__((used))
void jh_fault_handler_c(uint32_t *frame, uint32_t exc_return, uint32_t kind)
{
...
{
    if (s_exc.magic != JH_EXC_MAGIC || s_exc.kind == JH_FAULT_NONE) {
        return false;
    }
    dump_record(&s_exc);
    /* Consume the record so it is reported only once. */
    s_exc.magic = 0u;
