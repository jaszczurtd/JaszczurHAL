...
static hal_spi_settings_t spi_default_settings(void) {
    hal_spi_settings_t s = {HAL_SPI_CLOCK_DEFAULT_HZ, HAL_SPI_MSBFIRST, HAL_SPI_MODE0};
    return s;
}

static hal_spi_settings_t spi_normalize_settings(const hal_spi_settings_t *settings) {
...
static void spi_default_pins(uint8_t idx, uint8_t *rx_pin, uint8_t *tx_pin, uint8_t *sck_pin) {
    if (idx == 1u) {
...
static inline uint32_t spi_pin_port(uint8_t pin) { return (uint32_t)(pin >> 4); }
static inline uint32_t spi_pin_num(uint8_t pin)  { return (uint32_t)(pin & 0x0Fu); }
...
static uint32_t spi_prescaler_bits(uint32_t requested_hz, uint32_t source_hz, uint32_t *actual_hz) {
    if (requested_hz == 0u) {
...
static uint32_t spi_cr1_from_settings(const hal_spi_settings_t *settings, uint32_t source_hz,
                                      uint32_t *actual_hz) {
...
static void spi_hw_apply(uint8_t idx) {
    ...
    const uint32_t cr1 = spi_cr1_from_settings(&st->settings, spi_pclk_hz(idx), &actual_hz);
