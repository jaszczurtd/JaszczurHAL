...
static inline void wireguard_ensure_mutex(void) {
    if (s_wireguard_mutex == NULL) {
        hal_critical_section_enter();
        if (s_wireguard_mutex == NULL) {
            s_wireguard_mutex = hal_mutex_create();
        }
...
bool hal_wireguard_begin(const uint8_t local_ip[HAL_WIREGUARD_IPV4_OCTETS],
    ...
    const bool ok = s_wireguard.begin(ip_from_bytes(local_ip),
                                      private_key,
                                      remote_peer_address,
...
bool hal_wireguard_begin_advanced(const uint8_t local_ip[HAL_WIREGUARD_IPV4_OCTETS],
    ...
    if (!validate_non_empty(private_key, "hal_wireguard_begin_advanced", "private_key")) {
        return false;
    }
    if (!validate_non_empty(remote_peer_address, "hal_wireguard_begin_advanced", "remote_peer_address")) {
        return false;
    }
    if (!validate_non_empty(remote_peer_public_key, "hal_wireguard_begin_advanced", "remote_peer_public_key")) {
        return false;
    }
    if (remote_peer_port == 0u) {
    ...
    const bool ok = s_wireguard.beginAdvanced(ip_from_bytes(local_ip),
                                              private_key,
                                              remote_peer_address,
                                              remote_peer_public_key,
                                              remote_peer_port,
                                              ip_from_bytes(allowed_ip),
...
bool hal_wireguard_peer_up(char *endpoint_ip_out,
    ...
    if (endpoint_ip_out) {
        if (snprintf(endpoint_ip_out,
                     ...
                     "%u.%u.%u.%u",
                     (unsigned)endpoint_ip[0],
                     (unsigned)endpoint_ip[1],
                     (unsigned)endpoint_ip[2],
                     (unsigned)endpoint_ip[3]) < 0) {
...
bool hal_wireguard_kick_handshake(const uint8_t probe_ip[HAL_WIREGUARD_IPV4_OCTETS],
    ...
    const bool ok = s_wireguard.kickHandshake(ip_from_bytes(probe_ip),
                                              probe_port,
                                              min_interval_ms);
