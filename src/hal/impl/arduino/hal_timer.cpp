...
static void timer_ensure_mutex(void) {
    if (!s_pool_api_mutex) {
        ...
        if (!s_pool_api_mutex) {
            s_pool_api_mutex = hal_mutex_create();
        }
...
static uint16_t timer_cap_entries(uint16_t max_timers) {
    ...
    if (max_timers == 0u) return 1u;
    if (max_timers > 255u) return 255u;
    return max_timers;
}

static inline alarm_pool_t *resolve_pool(hal_timer_pool_t pool) {
    if (pool && pool->pool) return pool->pool;
...
hal_timer_pool_t hal_timer_pool_create(uint8_t hardware_alarm_num, uint16_t max_timers) {
    ...
    h->pool = alarm_pool_create((uint)hardware_alarm_num, (uint)timer_cap_entries(max_timers));
    if (!h->pool) {
        delete h;
        h = NULL;
    }
...
void hal_timer_pool_destroy(hal_timer_pool_t pool) {
    if (!pool || pool == HAL_TIMER_POOL_DEFAULT) return;
...
hal_alarm_id_t hal_timer_pool_add_alarm_us_ex(hal_timer_pool_t pool,
    ...
    alarm_id_t id = alarm_pool_add_alarm_in_us(
        target_pool, delay_us, (alarm_callback_t)callback, user_data, fire_if_past
    );
    if (id > 0) {
