...
static bool rp2040_init_transport(uint8_t pin, bool is800khz) {
    ...
    ws2812_program_init(pio, sm, offset, pin, is800khz ? 800000.0f : 400000.0f, 8u);
...
static bool rp2040_write_pixels(const uint8_t *pixels,
                                uint32_t num_bytes,
                                bool is800khz,
                                uint8_t pin,
                                void *user) {
    ...
    for (uint32_t i = 0; i < num_bytes; ++i) {
        pio_sm_put_blocking(s_transport.pio,
                            (uint)s_transport.pio_sm,
