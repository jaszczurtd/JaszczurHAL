...
class MCP_CAN_Guard {
public:
    explicit MCP_CAN_Guard(MCP_CAN &driver) : d(driver) { d.lock_driver_(); }
    ~MCP_CAN_Guard() { d.unlock_driver_(); }
private:
    MCP_CAN &d;
};

void MCP_CAN::lock_driver_(void)
{
    hal_mutex_lock(m_driver_mutex);
}

void MCP_CAN::unlock_driver_(void)
{
    hal_mutex_unlock(m_driver_mutex);
}

void MCP_CAN::spi_begin_(void)
{
...
{
    INT8U set = 1, cfg1 = 0, cfg2 = 0, cfg3 = 0;
...
MCP_CAN::MCP_CAN(SPIClass *_SPI, INT8U _CS)
{
    ...
    MCP2515_UNSELECT();
    pinMode(MCPCS, OUTPUT);
    mcpSPI = _SPI ? _SPI : &SPI;
    m_spiBus = (mcpSPI == &SPI) ? 0 : 1;
    m_driver_mutex = hal_mutex_create();
}
