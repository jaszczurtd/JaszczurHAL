...
static uint8_t spitft_hal_spi_bus_from_class(SPIClass *spi) {
...
inline void Adafruit_SPITFT::SPI_BEGIN_TRANSACTION(void) {
  if (connection == TFT_HARD_SPI) {
    if (!_halSpiLocked) {
      _halSpiBus = spitft_hal_spi_bus_from_class(hwspi._spi);
      hal_spi_lock(_halSpiBus);
      _halSpiLocked = true;
    }
#if defined(SPI_HAS_TRANSACTION)
...
#endif
  if ((connection == TFT_HARD_SPI) && _halSpiLocked) {
    hal_spi_unlock(_halSpiBus);
    _halSpiLocked = false;
  }
}
