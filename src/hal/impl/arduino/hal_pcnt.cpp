...
static hal_gpio_irq_mode_t to_irq_mode(hal_pcnt_edge_t edge) {
    switch (edge) {
        case HAL_PCNT_EDGE_FALLING: return HAL_GPIO_IRQ_FALLING;
        case HAL_PCNT_EDGE_BOTH:    return HAL_GPIO_IRQ_CHANGE;
        case HAL_PCNT_EDGE_RISING:
        default:                    return HAL_GPIO_IRQ_RISING;
    }
}
...
uint32_t hal_pcnt_read(uint8_t channel) {
    return (channel < RP2040_PCNT_CHANNELS && s_init[channel]) ? s_count[channel] : 0u;
}
