/**
 * MFRC522.h - HAL shared MFRC522 RFID driver.
 *
 * This JaszczurHAL driver is based on the MFRC522-spi-i2c-uart-async driver
 * by Makerspace Leiden, itself derived from the Miguel Balboa MFRC522/RFID
 * library and earlier contributors listed below.
 ...
 *UID changeable Mifare cards. Released into the public domain.
 *
 ...
 * 1) The micro controller running JaszczurHAL
...
public:
  ...
  void PCD_ReadRegister(MFRC522::PCD_Register reg, byte count, byte *values,
                        byte rxAlign = 0);
  ...
  static hal_status_t StatusCodeToHalStatus(StatusCode code);
...
public:
  MFRC522_I2C(byte resetPowerDownPin = UNUSED_PIN,
              byte chipAddress = MFRC522_I2C_DEFAULT_ADDR, byte bus = 0)
...
private:
  byte _resetPowerDownPin; // Optional, soft-rest will be used if set to
                           // UNUSEDPIN
...
public:
  MFRC522_SPI(const byte chipSelectPin,
              const byte resetPowerDownPin = UNUSED_PIN, byte bus = 0,
