...
static bool send_all(hal_tcp_socket_t socket, const void *data, size_t len) {
  const uint8_t *p = (const uint8_t *)data;
  size_t sent = 0u;
  while (sent < len) {
    int rc = hal_tcp_socket_send(socket, p + sent, len - sent);
    if (rc <= 0) {
      return false;
    }
    sent += (size_t)rc;
...
static bool append_text(char *out, size_t out_size, size_t *pos,
                        const char *text) {
