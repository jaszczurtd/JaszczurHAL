...
static bool max5395_validate(const hal_digipot_config_t *cfg) {
    return cfg != nullptr &&
           max5395_addr_valid(cfg->i2c_addr) &&
...
static bool max5395_set_resistance(const hal_digipot_config_t *cfg,
    ...
    switch (cfg->mode) {
        case HAL_DIGIPOT_MODE_VOLTAGE_DIVIDER:
            wiper = hal_digipot_scale_to_wiper_trunc(ohms,
                                                     cfg->e2e_resistance,
        ...
        case HAL_DIGIPOT_MODE_VARIABLE_RESISTOR_WH: {
            uint8_t config = 0u;
            if (!hal_digipot_i2c_read_cmd(cfg->i2c_bus, addr,
                                          MAX5395_CMD_CONFIG, &config)) {
                return false;
            }
            ...
            wiper = hal_digipot_scale_to_wiper_trunc(r - wiper_res,
                                                     cfg->e2e_resistance,
                                                     MAX5395_STEP_RESISTANCES);
...
static uint16_t max5395_step_count(void) {
    return MAX5395_STEP_RESISTANCES;
}
