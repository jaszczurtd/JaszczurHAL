...
/** Makefsdata can generate *all* files deflate-compressed (where file size
 * shrinks). Since nearly all browsers support this, this is a good way to
 * reduce ROM size. To compress the files, "miniz.c" must be downloaded
 * separately OR MAKEFS_SUPPORT_DEFLATE_ZLIB must be set and the zlib library
...
/* OUT_BUF_SIZE is the size of the output buffer used during decompression.
   OUT_BUF_SIZE must be a power of 2 >= TINFL_LZ_DICT_SIZE (because the
   low-level decompressor not only writes, but reads from the output buffer as
   it decompresses) */
...
#ifndef MAKEFS_SUPPORT_DEFLATE_ZLIB
/* tdefl_compressor contains all the state needed by the low-level compressor so
   it's a pretty big struct (~300k). This example makes it a global vs. putting
   it on the stack, of course in real-world usage you'll probably malloc() or
   new it. */
tdefl_compressor g_deflator;
#endif /* MAKEFS_SUPPORT_DEFLATE_ZLIB */

static int deflate_level; /* default compression level, can be changed via
                             command line */
...
/* Define this here since we don't include any external C files and ports might
 * override it */
...
static char
    file_buffer_c[COPY_BUFSIZE * 5 + ((COPY_BUFSIZE / HEX_BYTES_PER_LINE) * 3)];
...
static void print_usage(void) {
  printf(" Usage: htmlgen [targetdir] [-s] [-e] [-11] [-nossi] "
         "[-ssi:<filename>] [-c] [-f:<filename>] [-m] [-svr:<name>] "
  ...
  printf("   switch -s: toggle processing of subdirectories (default is "
         "on)" NEWLINE);
  printf("   switch -e: exclude HTTP header from file (header is created at "
         "runtime, default is off)" NEWLINE);
  printf("   switch -11: include HTTP 1.1 header (1.0 is default)" NEWLINE);
  printf("   switch -nossi: no support for SSI (cannot calculate "
         "Content-Length for SSI)" NEWLINE);
  printf("   switch -ssi: ssi filename (ssi support controlled by file list, "
         "not by extension)" NEWLINE);
  printf("   switch -c: precalculate checksums for all pages (default is "
         "off)" NEWLINE);
  printf("   switch -f: target filename (default is \"fsdata.c\")" NEWLINE);
  printf("   switch -m: include \"Last-Modified\" header based on file "
         "time" NEWLINE);
  printf("   switch -svr: server identifier sent in HTTP response header "
         "('Server' field)" NEWLINE);
  printf("   switch -x: comma separated list of extensions of files to exclude "
         "(e.g., -x:json,txt)" NEWLINE);
  printf("   switch -xc: comma separated list of extensions of files to not "
         "compress (e.g., -xc:mp3,jpg)" NEWLINE);
#if MAKEFS_SUPPORT_DEFLATE
  printf("   switch -defl: deflate-compress all non-SSI files (with opt. "
         "compr.-level, default=10)" NEWLINE);
  printf("                 ATTENTION: browser has to support "
         "\"Content-Encoding: deflate\"!" NEWLINE);
#endif
...
int main(int argc, char *argv[]) {
  ...
  printf(NEWLINE " makefsdata v" LWIP_VERSION_STRING
                 " - HTML to C source converter" NEWLINE);
  printf("     by Jim Pettinato               - circa 2003 " NEWLINE);
  printf("     extended by Simon Goldschmidt  - 2009 " NEWLINE NEWLINE);

  LWIP_ASSERT("sizeof(hdr_buf) must fit into an u16_t",
              sizeof(hdr_buf) <= 0xffff);
  ...
  for (i = 1; i < argc; i++) {
    ...
    if (argv[i][0] == '-') {
      if (strstr(argv[i], "-svr:") == argv[i]) {
        ...
        printf("Deflating all non-SSI files with level %d (but only if size is "
               "reduced)" NEWLINE,
               deflate_level);
#else
        ...
        printf("Skipping compression for files with extensions %s" NEWLINE,
               ncompress_list);
  ...
  if (!CHDIR_SUCCEEDED(CHDIR(path))) {
    /* if no subdir named 'fs' (or the one which was given) exists, spout usage
     * verbiage */
  ...
  curSubdir[0] =
      '\0'; /* start off in web page's root directory - relative paths */
  ...
  /* define FS_FILE_FLAGS_HEADER_INCLUDED to 1 if not defined (compatibility
   * with older httpd/fs) */
  fprintf(data_file,
          "#ifndef FS_FILE_FLAGS_HEADER_INCLUDED" NEWLINE
          "#define FS_FILE_FLAGS_HEADER_INCLUDED 1" NEWLINE "#endif" NEWLINE);
  /* define FS_FILE_FLAGS_HEADER_PERSISTENT to 0 if not defined (compatibility
   * with older httpd/fs: wasn't supported back then) */
  fprintf(data_file,
          "#ifndef FS_FILE_FLAGS_HEADER_PERSISTENT" NEWLINE
          "#define FS_FILE_FLAGS_HEADER_PERSISTENT 0" NEWLINE "#endif" NEWLINE);
  ...
  fprintf(
      ...
      "#ifndef FSDATA_FILE_ALIGNMENT" NEWLINE
      "#define FSDATA_FILE_ALIGNMENT 0" NEWLINE "#endif" NEWLINE);
#endif
  fprintf(data_file, "#ifndef FSDATA_ALIGN_PRE" NEWLINE
                     "#define FSDATA_ALIGN_PRE" NEWLINE "#endif" NEWLINE);
  fprintf(data_file, "#ifndef FSDATA_ALIGN_POST" NEWLINE
                     "#define FSDATA_ALIGN_POST" NEWLINE "#endif" NEWLINE);
#if ALIGN_PAYLOAD
  fprintf(data_file,
          "#if FSDATA_FILE_ALIGNMENT==2" NEWLINE
          "#include \"fsdata_alignment.h\"" NEWLINE "#endif" NEWLINE);
#endif
  ...
  fprintf(struct_file, "#define FS_NUMFILES %d" NEWLINE NEWLINE,
          filesProcessed);
  ...
  if (deflateNonSsiFiles) {
    printf("(Deflated total byte reduction: %d bytes -> %d bytes "
           "(%.02f%%)" NEWLINE,
           (int)overallDataBytes, (int)deflatedBytesReduced,
...
void concat_files(const char *file1, const char *file2,
    ...
    if (ret == 0) {
      ...
      for (i = 0; i < dir.n_files; i++) {
        ...
        if (ret == 0) {
          ...
          char currName[256];
          wcstombs_s(&num_char_converted, currName, sizeof(currName), file.name,
                     sizeof(currName));
#else
          ...
          if (freelen > 0) {
            ...
            if (!CHDIR_SUCCEEDED(CHDIR(".."))) {
              printf("Unable to get back to parent dir of: \"%s\"." NEWLINE,
                     currName);
              exit(-1);
            }
            curSubdir[sublen] = 0;
          } else {
            printf("WARNING: cannot process sub due to path length "
                   "restrictions: \"%s/%s\"\n",
                   curSubdir, currName);
          }
        }
      ...
      for (i = 0; i < dir.n_files; i++) {
        ...
        if (ret == 0) {
          if (!file.is_dir) {
            ...
            size_t num_char_converted;
            char curName[256];
            wcstombs_s(&num_char_converted, curName, sizeof(curName), file.name,
                       sizeof(curName));
#else
            ...
            if (file_to_exclude(curName)) {
              printf("skipping %s/%s by exclude list (-x option)..." NEWLINE,
                     curSubdir, curName);
              continue;
            }
...
static u8_t *get_file_data(const char *filename, int *file_size,
                           int can_be_compressed, int *is_compressed) {
  FILE *inFile;
  size_t fsize = 0;
  u8_t *buf;
  size_t r;
  ...
  if (deflateNonSsiFiles) {
    if (can_be_compressed) {
      if (fsize < OUT_BUF_SIZE) {
...
#ifndef MAKEFS_SUPPORT_DEFLATE_ZLIB
        /* create tdefl() compatible flags (we have to compose the low-level
         * flags ourselves, or use tdefl_create_comp_flags_from_zip_params() but
         * that means MINIZ_NO_ZLIB_APIS can't be defined). */
        mz_uint comp_flags =
            s_tdefl_num_probes[MZ_MIN(10, deflate_level)] |
        ...
        status = tdefl_compress(&g_deflator, next_in, &in_bytes, next_out,
                                &out_bytes, TDEFL_FINISH);
        ...
        if (out_bytes < fsize) {
          ...
          {
            /* sanity-check compression be inflating and comparing to the
             * original */
            ...
            dec_status = tinfl_decompress(
                &inflator, (const mz_uint8 *)ret_buf, &dec_in_bytes, s_checkbuf,
                (mz_uint8 *)next_out, &dec_out_bytes, 0);
            LWIP_ASSERT("tinfl_decompress failed",
                        dec_status == TINFL_STATUS_DONE);
#else  /* MAKEFS_SUPPORT_DEFLATE_ZLIB */
            int dec_status;
            dec_status =
...
#endif /* MAKEFS_SUPPORT_DEFLATE_ZLIB */
            LWIP_ASSERT("tinfl_decompress size mismatch",
                        fsize == dec_out_bytes);
            LWIP_ASSERT("decompressed memcmp failed",
          ...
          *file_size = out_bytes;
          printf(" - deflate: %d bytes -> %d bytes (%.02f%%)" NEWLINE,
                 (int)fsize, (int)out_bytes,
                 (float)((out_bytes * 100.0) / fsize));
          deflatedBytesReduced += (size_t)(fsize - out_bytes);
          *is_compressed = 1;
        } else {
          printf(" - uncompressed: (would be %d bytes larger using "
                 "deflate)" NEWLINE,
                 (int)(out_bytes - fsize));
        }
      } else {
...
static void process_file_data(FILE *data_file, u8_t *file_data,
                              size_t file_size) {
  ...
  for (i = 0; i < file_size; i++) {
    ...
    if ((++src_off % HEX_BYTES_PER_LINE) == 0) {
      LWIP_ASSERT("file_buffer_c overflow",
                  off < sizeof(file_buffer_c) - NEWLINE_LEN);
...
static int write_checksums(FILE *struct_file, const char *varname,
                           ...
                           const u8_t *file_data, size_t file_size) {
  ...
  fprintf(struct_file, "const struct fsdata_chksum chksums_%s[] = {" NEWLINE,
          varname);
...
static void register_filename(const char *qualifiedName) {
  struct file_entry *fe =
      (struct file_entry *)malloc(sizeof(struct file_entry));
...
int process_file(FILE *data_file, FILE *struct_file, const char *filename) {
  ...
  fprintf(data_file,
          ...
          varname, payload_alingment_dummy_counter++);
  fprintf(data_file, "#endif" NEWLINE);
#endif /* ALIGN_PAYLOAD */
  fprintf(data_file,
          "static const unsigned char FSDATA_ALIGN_PRE data_%s[] "
          "FSDATA_ALIGN_POST = {" NEWLINE,
          varname);
  /* encode source file name (used by file system, not returned to browser) */
  fprintf(data_file, "/* %s (%" SZT_F " chars) */" NEWLINE, qualifiedName,
  ...
  can_be_compressed =
      includeHttpHeader && !is_ssi && file_can_be_compressed(filename);
  file_data =
      get_file_data(filename, &file_size, can_be_compressed, &is_compressed);
  if (includeHttpHeader) {
    file_write_http_header(data_file, filename, file_size, &http_hdr_len,
                           &http_hdr_chksum, has_content_len, is_compressed);
  ...
  if (precalcChksum) {
    chksum_count = write_checksums(struct_file, varname, http_hdr_len,
                                   http_hdr_chksum, file_data, file_size);
  }

  /* build declaration of struct fsdata_file in temp file */
  fprintf(struct_file, "const struct fsdata_file file_%s[] = { {" NEWLINE,
          varname);
  ...
  i = 0;
  fprintf(data_file, NEWLINE "/* raw file data (%d bytes) */" NEWLINE,
          file_size);
...
int file_write_http_header(FILE *data_file, const char *filename, int file_size,
  ...
  fprintf(data_file, NEWLINE "/* \"%s\" (%" SZT_F " bytes) */" NEWLINE,
          cur_string, cur_len);
  ...
  if (provide_content_len) {
    ...
    fprintf(data_file, NEWLINE "/* \"%s%d\r\n\" (%" SZT_F "+ bytes) */" NEWLINE,
            cur_string, content_len, cur_len + 2);
    written += file_put_ascii(data_file, cur_string, cur_len, &i);
    if (precalcChksum) {
      memcpy(&hdr_buf[hdr_len], cur_string, cur_len);
      hdr_len += cur_len;
  ...
  if (is_compressed) {
    ...
    cur_len = strlen(cur_string);
    fprintf(data_file, NEWLINE "/* \"%s\" (%d bytes) */" NEWLINE, cur_string,
            cur_len);
    written += file_put_ascii(data_file, cur_string, cur_len, &i);
    i = 0;
  }
  ...
  if (precalcChksum) {
    LWIP_ASSERT("hdr_len + cur_len <= sizeof(hdr_buf)",
