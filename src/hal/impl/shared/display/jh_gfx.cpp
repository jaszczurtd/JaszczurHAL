...
JHGfx::JHGfx(int16_t w, int16_t h)
    : WIDTH(w), HEIGHT(h) {
    _width = WIDTH;
    _height = HEIGHT;
    rotation = 0;
...
void JHGfx::drawLine(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
                      uint16_t color) {
    if (x0 == x1) {
        if (y0 > y1) _jh_swap_int16(y0, y1);
        drawFastVLine(x0, y0, y1 - y0 + 1, color);
    } else if (y0 == y1) {
        if (x0 > x1) _jh_swap_int16(x0, x1);
...
void JHGfx::invertDisplay(bool i) {
    (void)i;
}
...
void JHGfx::fillCircleHelper(int16_t x0, int16_t y0, int16_t r,
...
void JHGfx::drawRoundRect(int16_t x, int16_t y, int16_t w, int16_t h,
...
void JHGfx::fillRoundRect(int16_t x, int16_t y, int16_t w, int16_t h,
                           int16_t r, uint16_t color) {
    int16_t max_radius = ((w < h) ? w : h) / 2;
    if (r > max_radius) r = max_radius;
    startWrite();
...
void JHGfx::fillTriangle(int16_t x0, int16_t y0, int16_t x1, int16_t y1,
    ...
    if (y0 > y1) { _jh_swap_int16(y0, y1); _jh_swap_int16(x0, x1); }
    if (y1 > y2) { _jh_swap_int16(y2, y1); _jh_swap_int16(x2, x1); }
    if (y0 > y1) { _jh_swap_int16(y0, y1); _jh_swap_int16(x0, x1); }
...
void JHGfx::drawBitmap(int16_t x, int16_t y, const uint8_t bitmap[],
...
void JHGfx::drawBitmap(int16_t x, int16_t y, uint8_t *bitmap,
                        int16_t w, int16_t h, uint16_t color) {
    int16_t byteWidth = (w + 7) / 8;
    uint8_t b = 0;
    startWrite();
...
void JHGfx::drawXBitmap(int16_t x, int16_t y, const uint8_t bitmap[],
                         int16_t w, int16_t h, uint16_t color) {
    int16_t byteWidth = (w + 7) / 8;
    uint8_t b = 0;
    startWrite();
...
void JHGfx::drawRGBBitmap(int16_t x, int16_t y, uint16_t *bitmap,
                           int16_t w, int16_t h) {
    startWrite();
    for (int16_t j = 0; j < h; j++, y++) {
        for (int16_t i = 0; i < w; i++) {
...
void JHGfx::drawChar(int16_t x, int16_t y, unsigned char c,
...
void JHGfx::charBounds(unsigned char c, int16_t *x, int16_t *y,
...
void JHGfx::getTextBounds(const char *str, int16_t x, int16_t y,
...
JHGfxCanvas1::~JHGfxCanvas1(void) {
    if (buffer && buffer_owned) free(buffer);
}
...
void JHGfxCanvas1::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
    if (h < 0) { h *= -1; y -= h - 1; if (y < 0) { h += y; y = 0; } }
    if ((x < 0) || (x >= width()) || (y >= height()) || ((y + h - 1) < 0)) return;
    if (y < 0) { h += y; y = 0; }
    if (y + h > height()) h = height() - y;

    if (getRotation() == 0) { drawFastRawVLine(x, y, h, color); }
    else if (getRotation() == 1) { int16_t t = x; x = WIDTH - 1 - y; y = t; x -= h - 1; drawFastRawHLine(x, y, h, color); }
    else if (getRotation() == 2) { x = WIDTH - 1 - x; y = HEIGHT - 1 - y; y -= h - 1; drawFastRawVLine(x, y, h, color); }
    else if (getRotation() == 3) { int16_t t = x; x = y; y = HEIGHT - 1 - t; drawFastRawHLine(x, y, h, color); }
}

void JHGfxCanvas1::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
    if (w < 0) { w *= -1; x -= w - 1; if (x < 0) { w += x; x = 0; } }
    if ((y < 0) || (y >= height()) || (x >= width()) || ((x + w - 1) < 0)) return;
    if (x < 0) { w += x; x = 0; }
    if (x + w >= width()) w = width() - x;

    if (getRotation() == 0) { drawFastRawHLine(x, y, w, color); }
    ...
    else if (getRotation() == 3) { int16_t t = x; x = y; y = HEIGHT - 1 - t; y -= w - 1; drawFastRawVLine(x, y, w, color); }
}

void JHGfxCanvas1::drawFastRawVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
    int16_t row_bytes = ((WIDTH + 7) / 8);
    uint8_t *ptr = &buffer[(x / 8) + y * row_bytes];
    if (color > 0) {
        uint8_t bit_mask = (0x80 >> (x & 7));
        for (int16_t i = 0; i < h; i++) { *ptr |= bit_mask; ptr += row_bytes; }
    } else {
        uint8_t bit_mask = ~(0x80 >> (x & 7));
        for (int16_t i = 0; i < h; i++) { *ptr &= bit_mask; ptr += row_bytes; }
    }
...
void JHGfxCanvas1::drawFastRawHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
    ...
    if ((x & 7) > 0) {
        ...
        if (color > 0) *ptr |= startByteBitMask;
        else *ptr &= ~startByteBitMask;
        ptr++;
    }
    ...
    if (remainingWidthBits > 0) {
        ...
        if (lastByteBits > 0) {
            ...
            if (color > 0) *ptr |= lastByteBitMask;
            else *ptr &= ~lastByteBitMask;
        }
    }
}
...
void JHGfxCanvas8::fillScreen(uint16_t color) {
    if (buffer) memset(buffer, color, (size_t)WIDTH * HEIGHT);
}

void JHGfxCanvas8::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
    if (h < 0) { h *= -1; y -= h - 1; if (y < 0) { h += y; y = 0; } }
    if ((x < 0) || (x >= width()) || (y >= height()) || ((y + h - 1) < 0)) return;
    if (y < 0) { h += y; y = 0; }
    if (y + h > height()) h = height() - y;
...
void JHGfxCanvas8::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
    if (w < 0) { w *= -1; x -= w - 1; if (x < 0) { w += x; x = 0; } }
    if ((y < 0) || (y >= height()) || (x >= width()) || ((x + w - 1) < 0)) return;
    if (x < 0) { w += x; x = 0; }
    if (x + w >= width()) w = width() - x;
...
void JHGfxCanvas8::drawFastRawVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
    uint8_t *buffer_ptr = buffer + (size_t)y * WIDTH + x;
    for (int16_t i = 0; i < h; i++) { *buffer_ptr = color; buffer_ptr += WIDTH; }
}

void JHGfxCanvas8::drawFastRawHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
...
void JHGfxCanvas16::fillScreen(uint16_t color) {
    if (buffer) {
        if (color == 0) {
            ...
            for (uint32_t i = 0; i < n; i++) buffer[i] = color;
        }
    }
}
...
void JHGfxCanvas16::drawFastVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
    if (h < 0) { h *= -1; y -= h - 1; if (y < 0) { h += y; y = 0; } }
    if ((x < 0) || (x >= width()) || (y >= height()) || ((y + h - 1) < 0)) return;
    if (y < 0) { h += y; y = 0; }
    if (y + h > height()) h = height() - y;
...
void JHGfxCanvas16::drawFastHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
    if (w < 0) { w *= -1; x -= w - 1; if (x < 0) { w += x; x = 0; } }
    if ((y < 0) || (y >= height()) || (x >= width()) || ((x + w - 1) < 0)) return;
    if (x < 0) { w += x; x = 0; }
    if (x + w >= width()) w = width() - x;
...
void JHGfxCanvas16::drawFastRawVLine(int16_t x, int16_t y, int16_t h, uint16_t color) {
    uint16_t *buffer_ptr = buffer + (size_t)y * WIDTH + x;
    for (int16_t i = 0; i < h; i++) { *buffer_ptr = color; buffer_ptr += WIDTH; }
}

void JHGfxCanvas16::drawFastRawHLine(int16_t x, int16_t y, int16_t w, uint16_t color) {
    uint16_t *buffer_ptr = buffer + (size_t)y * WIDTH + x;
    for (int16_t i = 0; i < w; i++) { buffer_ptr[i] = color; }
}
