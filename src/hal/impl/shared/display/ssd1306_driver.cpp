...
bool jh_ssd1306_init(jh_ssd1306_t *dev, const jh_ssd1306_config_t *config) {
    ...
    dev->i2c_addr = config->i2c_addr ? config->i2c_addr
                                     : default_address(config->height);
    ...
    const uint8_t vcc = dev->config.vccstate ? dev->config.vccstate
                                             : JH_SSD1306_SWITCHCAPVCC;
    ...
    ok = ok && ssd1306_command(dev, (vcc == JH_SSD1306_EXTERNALVCC) ? 0x10u : 0x14u);
    ...
    ok = ok && ssd1306_command(dev, (vcc == JH_SSD1306_EXTERNALVCC) ? 0x22u : 0xF1u);
...
bool jh_ssd1306_display(jh_ssd1306_t *dev, const uint8_t *buffer) {
    ...
    while (remaining > 0u) {
        size_t chunk = remaining < SSD1306_I2C_CHUNK ? remaining
                                                     : SSD1306_I2C_CHUNK;
        hal_i2c_begin_transmission_bus(dev->config.bus, dev->i2c_addr);
