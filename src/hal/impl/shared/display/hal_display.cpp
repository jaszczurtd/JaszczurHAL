...
#elif defined(HAL_DISPLAY_ST7735) || defined(HAL_DISPLAY_ST7789) || defined(HAL_DISPLAY_ST7796S)
#include "st77xx_driver.h"
#else
#error "hal_display requires a supported HAL_DISPLAY_* TFT backend when HAL_ENABLE_TFT is set."
#endif
...
static bool fill_rect_unlocked(int x, int y, int w, int h, uint16_t color) {
    ...
    return tft_fill_rect_driver((uint16_t)x, (uint16_t)y,
...
bool hal_display_init_ssd1306_i2c(int width, int height, uint8_t i2c_addr,
                                  ...
                                  bool periphBegin) {
    return hal_display_init_ssd1306_i2c_ex(width, height, 0u, i2c_addr,
                                           rst_pin, switchvcc, periphBegin);
}
...
bool hal_display_draw_rgb_bitmap(int x, int y, uint16_t *data, int w, int h) {
    ...
    if (using_tft()) {
        if (x < 0 || y < 0 || x + w > s_width || y + h > s_height) {
            hal_derr("hal_display_draw_rgb_bitmap: out-of-bounds clipping is not supported");
            return false;
        }
        return tft_draw_rgb_bitmap_driver((uint16_t)x, (uint16_t)y,
                                          data, (uint16_t)w, (uint16_t)h);
    }
#else
    (void)x;
    (void)y;
#endif
    hal_derr("hal_display_draw_rgb_bitmap: RGB bitmaps are unsupported on current backend");
    return false;
}
...
bool hal_display_clear_text_line(int line_index, int line_height, uint16_t bg_color) {
    ...
    return hal_display_fill_rect(0, line_index * line_height, width, line_height, bg_color);
}
...
int hal_display_prepare_text(char *display_txt, size_t display_txt_size,
    ...
    const int w = hal_display_prepare_text_v(display_txt, display_txt_size, format, args);
    va_end(args);
    return w;
}
