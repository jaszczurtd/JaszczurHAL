...
static uint8_t resolution_to_config(uint8_t resolution) {
    switch (resolution) {
        case 12u: return DS18B20_CONFIG_12_BIT;
        case 11u: return DS18B20_CONFIG_11_BIT;
        case 10u: return DS18B20_CONFIG_10_BIT;
        case 9u:
        default:  return DS18B20_CONFIG_9_BIT;
    }
}
...
static bool ds18b20_is_connected(hal_ds18b20_impl_t *h,
                                 const uint8_t *address,
                                 uint8_t scratchpad[9]) {
    if (scratchpad == NULL) {
        uint8_t tmp[9] = {0};
    ...
    return ok &&
           !scratchpad_is_all_zeros(scratchpad, 9u) &&
...
static uint8_t ds18b20_get_resolution(hal_ds18b20_impl_t *h,
    ...
    if (ds18b20_is_connected(h, address, scratchpad)) {
        ...
        switch (scratchpad[DS18B20_SCRATCH_CONFIG]) {
            case DS18B20_CONFIG_12_BIT: return 12u;
            case DS18B20_CONFIG_11_BIT: return 11u;
            case DS18B20_CONFIG_10_BIT: return 10u;
            case DS18B20_CONFIG_9_BIT:  return 9u;
            default:                    break;
        }
    }
...
static bool ds18b20_get_address(hal_ds18b20_impl_t *h,
                                uint8_t *address,
                                uint8_t index) {
...
static bool resolve_sensor_address(hal_ds18b20_impl_t *h) {
    if (h->use_rom) {
        if (!ds18b20_valid_address(h->rom) ||
            !ds18b20_valid_family(h->rom) ||
    ...
    if (h->ds18_count > 1u) {
        hal_derr("hal_ds18b20_init: multiple sensors on pin %u; using first discovered address",
                 (unsigned)h->pin);
    }
...
static int32_t ds18b20_calculate_temperature(const uint8_t *address,
    ...
    if ((address[0] == DS18B20_FAMILY_DS1825) &&
        ...
        fp_temperature = (((int32_t)scratchpad[DS18B20_SCRATCH_TEMP_MSB]) << 11) |
                         (((int32_t)scratchpad[DS18B20_SCRATCH_TEMP_LSB] & 0xFC) << 3) |
                         neg;
    } else {
    ...
    if ((address[0] == DS18B20_FAMILY_DS18S20) &&
        ...
        fp_temperature =
            (((fp_temperature & 0xFFF0) << 3) - 32 +
             (((scratchpad[DS18B20_SCRATCH_COUNT_PER_C] -
                scratchpad[DS18B20_SCRATCH_COUNT_REMAIN]) << 7) /
              scratchpad[DS18B20_SCRATCH_COUNT_PER_C])) | neg;
    }
...
static float ds18b20_get_temp_c(hal_ds18b20_impl_t *h,
                                const uint8_t *address) {
    uint8_t scratchpad[9] = {0};
    if (ds18b20_is_connected(h, address, scratchpad)) {
        return ds18b20_raw_to_celsius(ds18b20_calculate_temperature(address, scratchpad));
    }
    return -127.0f;
}
