...
private:
  ...
  hal_mutex_t _mutex = NULL;

  uint8_t _rs_pin = 0u;             // false: command. true: character.
  uint8_t _rw_pin = HD44780_NO_PIN; // false: write. true: read.
  uint8_t _enable_pin = 0u;         // activated by a high pulse.
  uint8_t _data_pins[8] = {};

  uint8_t _displayfunction = 0u;
  uint8_t _displaycontrol = 0u;
  uint8_t _displaymode = 0u;

  uint8_t _initialized = 0u;

  uint8_t _numlines = 0u;
  uint8_t _row_offsets[4] = {};
  uint8_t _currow = 0u; // tracks the active row so println() can advance lines
};
