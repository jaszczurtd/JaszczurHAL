...
#if HAL_TARGET_IS_RP
constexpr hal_eeprom_type_t kFlashType = HAL_EEPROM_RP2040;
constexpr size_t kFlashMirrorSize = HAL_RP_FLASH_EEPROM_SIZE;
#else
constexpr hal_eeprom_type_t kFlashType = HAL_EEPROM_STM32_FLASH;
constexpr size_t kFlashMirrorSize = HAL_STM32_FLASH_EEPROM_SIZE;
#endif
constexpr uint16_t kAt24c256Size = 32768u;

hal_eeprom_type_t s_type = HAL_EEPROM_AT24C256;
uint16_t s_size = kAt24c256Size;
...
static uint8_t s_flash_mirror[kFlashMirrorSize];
#if HAL_TARGET_IS_RP
...
static hal_status_t flash_commit_mirror(void) {
  ...
  bool ok = true;
  for (uint32_t offset = 0u; offset < s_flash_reserved_size;
  ...
  for (uint32_t offset = 0u; ok && offset < s_flash_reserved_size;
       offset += 8u) {
    bool all_erased = true;
    for (uint8_t i = 0u; i < 8u; ++i) {
      if (s_flash_mirror[offset + i] != 0xFFu) {
        all_erased = false;
        break;
      }
    }
    if (!all_erased && !jh_stm32g474_flash_program_doubleword(
                           s_flash_start + offset, &s_flash_mirror[offset])) {
      ok = false;
    }
    notify_progress();
  }
